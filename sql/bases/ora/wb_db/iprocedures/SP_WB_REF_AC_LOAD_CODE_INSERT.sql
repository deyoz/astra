create or replace procedure SP_WB_REF_AC_LOAD_CODE_INSERT
(cXML_in in clob,
   cXML_out out clob)
as
id number:=-1;
...
P_PRIORITY number:=0;
P_IATA_CODE_ID number:=-1;
P_DENSITY_INT_PART varchar2(50);
P_DENSITY_DEC_PART varchar2(50);
P_DENSITY number;
...
R_COUNT int:=0;
  begin
    ...
    select to_number(extractValue(xmltype(cXML_in),'/root[1]/list[1]/P_PRIORITY[1]')) into P_PRIORITY from dual;
    select to_number(extractValue(xmltype(cXML_in),'/root[1]/list[1]/P_IATA_CODE_ID[1]')) into P_IATA_CODE_ID from dual;
    ...
    select extractValue(xmltype(cXML_in),'/root[1]/list[1]/P_DENSITY_INT_PART[1]') into P_DENSITY_INT_PART from dual;
    select extractValue(xmltype(cXML_in),'/root[1]/list[1]/P_DENSITY_DEC_PART[1]') into P_DENSITY_DEC_PART from dual;

    P_DENSITY:=to_number(P_DENSITY_INT_PART||
                           '.'||
                             P_DENSITY_DEC_PART);
    ...
    if str_msg is null then
      begin
         ...
         where ID=P_IATA_CODE_ID;

         if r_count=0 then
           begin
             if P_LANG='ENG' then
               begin
                 str_msg:='This IATA Load Code is removed!';
               end;
             else
               begin
                 str_msg:='Запись для такого IATA Load Code удалена из справочника!';
               end;
             end if;
           end;
          end if;
    ...
    if (str_msg is null) then
      begin
        id:=SEC_WB_REF_AIRCO_LOAD_CODES.nextval();

        insert into WB_REF_AIRCO_LOAD_CODES (ID,
                                               ID_AC,
	                                               CODE_NAME,
	                                                 IATA_CODE_ID,
	                                                   DENSITY,
    ...
    commit;
  end SP_WB_REF_AC_LOAD_CODE_INSERT;
/
