create or replace procedure SP_WB_REF_AC_CLASS_CODE_INSERT
(cXML_in in clob,
   cXML_out out clob)
as
id number:=-1;
...
R_COUNT int:=0;
  begin
    ...
    if (str_msg is null) then
      begin
        id:=SEC_WB_REF_AIRCO_CLASS_CODES.nextval();

        insert into WB_REF_AIRCO_CLASS_CODES (ID,
                                                ID_AC,
	                                                CLASS_CODE,
	                                                  DATE_FROM,
	                                                    DATE_TO,
    ...
    commit;
  end SP_WB_REF_AC_CLASS_CODE_INSERT;
/
