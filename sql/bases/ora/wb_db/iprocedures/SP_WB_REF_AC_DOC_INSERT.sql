create or replace procedure SP_WB_REF_AC_DOC_INSERT
(cXML_in in clob,
   cXML_out out clob)
as
P_AIRCO_ID number:=-1;
...
DEL_REF_NAME varchar2(200);
  begin
    ...
    select distinct f.id,
                      f.copy_count,
                        f.doc_name
    from (select to_number(extractValue(value(t),'list/P_DOC_ID[1]')) as id,
                 to_number(extractValue(value(t),'list/P_COPY_COUNT[1]')) as copy_count,
                 extractValue(value(t),'list/P_DOC_NAME[1]') as doc_name
    ...
    if (str_msg is null) then
      begin
        p_id:=SEC_WB_REF_AIRCO_AUTO_DOCS_ADV.nextval();

        insert into WB_REF_AIRCO_AUTO_DOCS_ADV (ID,
	                                                ID_AC,
	                                                  DATE_FROM,
	                                                    REMARK,
	                                                      U_NAME,
        ...
        insert into WB_REF_AIRCO_AUTO_DOCS_DET (ID,
	                                                ID_AC,
	                                                  ADV_ID,
	                                                    DOC_ID,
	                                                      U_NAME,
	                                                        U_IP,
	                                                          U_HOST_NAME,
	                                                            DATE_WRITE,
	                                                              COPY_COUNT)
        select SEC_WB_REF_AIRCO_AUTO_DOCS_DET.nextval,
                 P_AIRCO_ID,
                   P_ID,
                     ID,
                       P_U_NAME,
