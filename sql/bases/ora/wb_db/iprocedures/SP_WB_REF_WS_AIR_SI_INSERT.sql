create or replace procedure SP_WB_REF_WS_AIR_SI_INSERT
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
...
R_COUNT int:=0;
  begin
    ...
    if (str_msg is null) then
      begin
        P_ID:=SEC_WB_REF_WS_AIR_SUP_ADV.nextval();

        insert into WB_REF_WS_AIR_SUPPL_INFO_ADV (ID,
                                                    ID_AC,
	                                                    ID_WS,
	                                                      ID_BORT,
	                                                        DATE_FROM,
    ...
    commit;
  end SP_WB_REF_WS_AIR_SI_INSERT;
/
