create or replace procedure SP_WB_REF_AC_MES_INSERT
(cXML_in in clob,
   cXML_out out clob)
as
P_AIRCO_ID number:=-1;
...
DEL_REF_NAME varchar2(200);
  begin
    ...
    if (str_msg is null) then
      begin
        p_id:=SEC_WB_REF_AIRCO_AUTO_MES_ADV.nextval();

        insert into WB_REF_AIRCO_AUTO_MES_ADV (ID,
	                                               ID_AC,
	                                                 DATE_FROM,
	                                                   REMARK,
	                                                     U_NAME,
