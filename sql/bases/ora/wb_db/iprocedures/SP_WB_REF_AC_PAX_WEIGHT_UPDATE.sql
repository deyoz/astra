create or replace procedure SP_WB_REF_AC_PAX_WEIGHT_UPDATE
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
...
R_COUNT int:=0;
  begin
         ...
         where ID_AC=P_AIRCO_ID and
               ID_CLASS=P_CLASS_ID and
                nvl(DATE_FROM, to_date('01.01.1900', 'dd.mm.yyyy'))=
                   nvl(P_DATE_FROM, to_date('01.01.1900', 'dd.mm.yyyy')) and
               id<>P_ID;
    ...
    if (str_msg is null) then
      begin
        ...
        set DESCRIPTION=P_DESCRIPTION,
	          ...
	          INFANT=P_INFANT,
	          HAND_BAG=P_HAND_BAG,
	          HAND_BAG_INCLUDE=P_HAND_BAG_INCLUDE,
	          U_NAME=P_U_NAME,
	          U_IP=P_U_IP,
    ...
    commit;
  end SP_WB_REF_AC_PAX_WEIGHT_UPDATE;
/
