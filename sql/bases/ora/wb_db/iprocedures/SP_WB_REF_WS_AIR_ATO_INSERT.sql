create or replace procedure SP_WB_REF_WS_AIR_ATO_INSERT
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
...
R_COUNT int:=0;
  begin
    ...
    if (str_msg is null) then
      begin
        P_ID:=SEC_WB_REF_WS_AIR_ATO.nextval();

        insert into WB_REF_WS_AIR_ATO (ID,
                                         ID_AC,
	                                         ID_WS,
	                                           ID_BORT,
	                                             DATE_FROM,
	                                               SORT_CONTAINER_LOADING,
	                                                 EZFW_THRESHOLD_LIMIT,
	                                                   EZFW_THRESHOLD_LIMIT_VAL,
	                                                     CHECK_LATERAL_IMBALANCE_LIMITS,
	                                                       PTO_CLASS_TRIM,
	                                                         PTO_CABIN_AREA_TRIM,
	                                                           PTO_SEAT_ROW_TRIM,
	                                                             REMARK,
        ...
        select P_ID,
                 P_ID_AC,
	                 P_ID_WS,
	                   P_ID_BORT,
	                     P_DATE_FROM,
	                       P_SORT_CONTAINER_LOADING,
	                         P_EZFW_THRESHOLD_LIMIT,
	                           P_EZFW_T_LIMIT_VAL,
	                             P_CHECK_LATERAL_I_LIMITS,
	                               P_PTO_CLASS_TRIM,
	                                 P_PTO_CABIN_AREA_TRIM,
	                                   P_PTO_SEAT_ROW_TRIM,
	                                     P_REMARK,
	                                       P_U_NAME,
	                                         P_U_IP,
    ...
    commit;
  end SP_WB_REF_WS_AIR_ATO_INSERT;
/
