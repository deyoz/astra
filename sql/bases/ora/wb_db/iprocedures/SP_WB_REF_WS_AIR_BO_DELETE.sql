create or replace procedure SP_WB_REF_WS_AIR_BO_DELETE
(cXML_in in clob,
   cXML_out out clob)
as
P_LANG varchar2(50):='';
...
str_msg clob:=null;
  begin
    ...
    where exists(select 1
                 ...
                 where t.id=WB_REF_WS_AIR_BAL_OUTPUT_ADV.id);
    ...
    where exists(select 1
                 from WB_TEMP_XML_ID t
                 where t.id=WB_REF_WS_AIR_BAL_OUTPUT_ADV_I.adv_id);
    ...
    where exists(select 1
                 from WB_TEMP_XML_ID t
                 where t.id=WB_REF_WS_AIR_BAL_OUTPUT_DATA.adv_id);
    ...
    commit;
  end SP_WB_REF_WS_AIR_BO_DELETE;
/
