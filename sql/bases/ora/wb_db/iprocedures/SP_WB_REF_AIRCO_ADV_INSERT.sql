create or replace procedure SP_WB_REF_AIRCO_ADV_INSERT
(cXML_in in clob,
   cXML_out out clob)
as
id number:=-1;
...
str_msg clob:=null;
  begin
    ...
    if str_msg is null then
      begin
         ...
         where ID_AC=P_AIRCO_ID and
               DATE_FROM=to_date(P_DATE_FROM_D||'.'||P_DATE_FROM_M||'.'||P_DATE_FROM_Y, 'dd.mm.yyyy');
    ...
    */
    ...
    if (str_msg is null) then
      begin
        id:=SEC_WB_REF_AIRCOMPANY_ADV_INFO.nextval();
    ...
    commit;
  end SP_WB_REF_AIRCO_ADV_INSERT;
/
