create or replace procedure SP_WB_REF_AC_C_DATA_1_INSERT
(cXML_in in clob,
   cXML_out out clob)
as
id number:=-1;
...
r_count int:=0;
  begin
   ...
   -- select extractValue(xmltype(cXML_in),'/root[1]/list[1]/P_REMARK[1]') into P_REMARK from dual;
    ...
    if (str_msg is null) then
      begin
        id:=SEC_WB_REF_AIRCO_C_DATA_1.nextval();

        insert into WB_REF_AIRCO_C_DATA_1 (ID,
                                             ID_AC,
	                                             DATE_FROM,
	                                               H_ADRESS,
	 	                                               C_PERSONE,
    ...
    commit;
  end SP_WB_REF_AC_C_DATA_1_INSERT;
/
