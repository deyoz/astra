create or replace procedure SP_WB_REF_WS_AIR_TYPE_INSERT
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
...
R_COUNT int:=0;
  begin
    ...
    if (str_msg is null) then
      begin
        P_ID:=SEC_WB_REF_WS_AIR_TYPE.nextval();

        insert into WB_REF_WS_AIR_TYPE (ID,
                                          ID_AC,
	                                          ID_WS,
	                                            ID_BORT,
	                                              DATE_FROM,
                 ...
                 P_ID_AC,
	                 P_ID_WS,
	                   P_ID_BORT,
	                     P_DATE_FROM,
	                       P_REVISION,
	                         P_ID_WS_TRANSP_KATEG,
	                           P_ID_WS_TYPE_OF_LOADING,
	                             P_UPPER_DECK,
	                               P_MAIN_DECK,
	                                 P_LOWER_DECK,
	                                   P_REMARK,
    ...
    commit;
  end SP_WB_REF_WS_AIR_TYPE_INSERT;
/
