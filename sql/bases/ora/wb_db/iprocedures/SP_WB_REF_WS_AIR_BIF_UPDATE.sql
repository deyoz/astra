create or replace procedure SP_WB_REF_WS_AIR_BIF_UPDATE
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
...
R_COUNT int:=0;
  begin
    ...
    if (str_msg is null) then
      begin
        update  WB_REF_WS_AIR_BAS_IND_FORM
        set DATE_FROM=P_DATE_FROM,
	          REF_ARM=P_REF_ARM,
	          K_CONST=P_K_CONST,
	          C_CONST=P_C_CONST,
	          LEN_MAC_RC=P_LEN_MAC_RC,
	          LEMAC_LERC=P_LEMAC_LERC,
	          U_NAME=P_U_NAME,
	          U_IP=P_U_IP,
    ...
    commit;
  end SP_WB_REF_WS_AIR_BIF_UPDATE;
/
