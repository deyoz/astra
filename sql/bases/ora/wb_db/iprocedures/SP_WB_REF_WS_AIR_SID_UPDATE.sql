create or replace procedure SP_WB_REF_WS_AIR_SID_UPDATE
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
  ...
  begin
    ...
    if str_msg is null then
      begin
        ...
        where not exists(select 1
                         from WB_REF_WS_AIR_SUPPL_INFO_ITEM i
                         where i.id=t.id);
    ...
    if str_msg is null then
      begin
        ...
        from WB_TEMP_XML_ID t
        where not exists(select 1
                         from WB_REF_WS_AIR_SUP_INFO_DET_COL i
                         where i.id=t.num);
    ...
    if (str_msg is null) then
      begin
        update WB_REF_WS_AIR_SUPPL_INFO_ADV
		                                       ...
		                                       i.REMARK,
		                                         i.U_NAME,
		                                           i.U_IP,
		                                             i.U_HOST_NAME,
		                                               i.DATE_WRITE
        from WB_REF_WS_AIR_SUPPL_INFO_ADV_I i
        where i.adv_id=P_ID;
		                                                               ...
		                                                               i.REMARK,
		                                                                 i.U_NAME,
		                                                                   i.U_IP,
                                                                             ...
                                                                             i.ADV_ID,
                                                                               i.ADV_ID
        from WB_REF_WS_AIR_SUPPL_INFO_DATA i
        where i.adv_id=P_ID;

        delete
        from WB_REF_WS_AIR_SUPPL_INFO_DATA
        where adv_id=P_ID;
        ------------------------------------------------------------------------
        ------------------------------------------------------------------------
        insert into WB_REF_WS_AIR_SUPPL_INFO_ADV_I (ID,
                                                      ID_AC,
	                                                      ID_WS,
	                                                        ID_BORT,
	                                                          DATE_FROM,	
                                                                  ...
                                                                  REMARK,
                                                                    U_NAME,
	                                                                    U_IP,
	                                                                      U_HOST_NAME,
	                                                                        DATE_WRITE)
        select SEC_WB_REF_WS_AIR_SUP_ADV_I.nextval,
                 P_ID_AC,
	                 P_ID_WS,
	                   P_ID_BORT,
	                     P_DATE_FROM,
        ...
        insert into WB_REF_WS_AIR_SUPPL_INFO_DATA (ID,
                                                     ID_AC,
	                                                     ID_WS,
	                                                       ID_BORT,
	                                                         DATE_FROM,	
                                                                     ...
                                                                     U_NAME,
	                                                                     U_IP,
	                                                                       U_HOST_NAME,
	                                                                         DATE_WRITE,
                                                                             adv_id)
         select SEC_WB_REF_WS_AIR_SUP_DATA.nextval,
                  P_ID_AC,
	                  P_ID_WS,
	                    P_ID_BORT,
	                      P_DATE_FROM,
    ...
    commit;
  end SP_WB_REF_WS_AIR_SID_UPDATE;
/
