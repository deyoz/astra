create or replace procedure SP_WB_REF_WS_AIR_MEAS_UPDATE
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
...
R_COUNT int:=0;
  begin
    ...
    if (str_msg is null) then
      begin
        update WB_REF_WS_AIR_MEASUREMENT
        set DATE_FROM=P_DATE_FROM,
	          WEIGHT_ID=P_MEAS_WEIGHT,
	          VOLUME_ID_LIQUID=P_MEAS_LIQUID_VOLUME,
	          VOLUME_ID=P_MEAS_VOLUME,
	          LENGTH_ID=P_MEAS_LENGTH,
	          DENSITY_ID_FUEL=P_MEAS_FUEL_DENSITY,
	          MOMENTS_ID=P_MEAS_MOMENTS,
	          REMARK=P_REMARK,
	          U_NAME=P_U_NAME,
	          U_IP=P_U_IP,
    ...
    commit;
  end SP_WB_REF_WS_AIR_MEAS_UPDATE;
/
