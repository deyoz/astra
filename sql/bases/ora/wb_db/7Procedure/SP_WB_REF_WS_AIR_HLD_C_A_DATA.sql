create or replace procedure SP_WB_REF_WS_AIR_HLD_C_A_DATA
(cXML_in in clob,
   cXML_out out clob)
as
P_ID_AC number:=-1;
...
begin
      ...
      begin
        ...
        if (V_IS_AC>0) and
             (V_IS_WS>0) and
               (V_IS_BORT>0) then
          begin
            insert into WB_REF_WS_AIR_HLD_CMP_A (ID,
	                                                 ID_AC,
	                                                   ID_WS,
	                                                     ID_BORT,
	                                                       CH_BALANCE_ARM,
	                                                           ...
	                                                           REMARKS,
	                                                             U_NAME,
	                                                               U_IP,
	                                                                 U_HOST_NAME,
	                                                                   DATE_WRITE)
            select SEC_WB_REF_WS_AIR_HLD_CMP_A.nextval,
                     P_ID_AC,
                       P_ID_WS,
                         P_ID_BORT,
                           0,
    ...
    if V_R_COUNT>0 then
      begin
        ...
        INTO cXML_data
        from WB_REF_WS_AIR_HLD_CMP_A i
...
end SP_WB_REF_WS_AIR_HLD_C_A_DATA;
/
