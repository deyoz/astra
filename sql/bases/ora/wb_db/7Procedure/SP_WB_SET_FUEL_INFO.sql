create or replace procedure SP_WB_SET_FUEL_INFO
(cXML_in in clob,
   cXML_out out clob)
as
P_ELEM_ID number:=-1;
...
begin
  ...
  select replace(cXML_in, '"set_fuel_info"', '"get_fuel_info"') into cXML_out from dual;
...
<!--
    Ответ.
-->
<root name="set_fuel_info" result="ok">
</root>';
  commit;
end SP_WB_SET_FUEL_INFO;
/
