create or replace PROCEDURE SP_WBA_GET_LOADSHEET
...
BEGIN
  ...
  select t1.ID_AC, t1.ID_WS, t1.ID_BORT, t1.NR, to_char(t1.S_DTL_1, 'DDMONYY', 'nls_date_language=English'), to_char(t1.S_DTL_1, 'HH24MI'), to_char(t1.S_DTL_1, 'DD') -- , t1.ID_SL -- на будущее - конфигурация
  into vID_AC, vID_WS, vID_BORT, cFlight_3a, cDate_7, cTime_8, cIdentifier_3b -- , vID_SL -- конфигурация - на будущее
  from WB_SHED t1
  where t1.ID = P_ID;
  ...
  */
  ...
  begin
    ...
    where t1.ID_WS = vID_WS and t1.ID_BORT = vID_BORT -- and t2.ID = vID_SL -- конфигурация - на будущее
      and rownum <= 1;
