create or replace procedure SP_WB_REF_WS_AIR_DOW_CR_C_UPD
(cXML_in in clob,
   cXML_out out clob)
as
...
P_CR_CODE_NAME varchar2(200):='';
P_FC_NUMBER NUMBER:=null;
P_CC_NUMBER NUMBER:=null;
...
V_R_COUNT int:=0;
  begin
    ...
    select extractValue(xmltype(cXML_in),'/root[1]/list[1]/P_CR_CODE_NAME[1]') into P_CR_CODE_NAME from dual;
    ...
    select to_number(extractValue(xmltype(cXML_in),'/root[1]/list[1]/P_FC_NUMBER[1]')) into P_FC_NUMBER from dual;
    select to_number(extractValue(xmltype(cXML_in),'/root[1]/list[1]/P_CC_NUMBER[1]')) into P_CC_NUMBER from dual;
    ...
    if V_STR_MSG is null then
      begin
        ...
        where PHRASE=P_CR_CODE_NAME;

        if V_R_COUNT>0 then
          begin
            if P_LANG='ENG' then
              begin
                 V_STR_MSG:='Value '||P_CR_CODE_NAME||' is a phrase reserved!';
              end;
            else
              begin
                V_STR_MSG:='Значение '||P_CR_CODE_NAME||' является зарезервированной фразой!';
              end;
            end if;
          end;
        end if;
    ...
    if V_STR_MSG is null then
      begin
        ...
        where id<>P_ID and
              ...
              CR_CODE_NAME=P_CR_CODE_NAME;

      if V_R_COUNT>0 then
          begin
            if P_LANG='ENG' then
              begin
                 V_STR_MSG:='Recording with the value of the field "Crew Code Name" already exists!';
              end;
            else
              begin
                V_STR_MSG:='Запись с таким значением поля "Crew Code Name" уже существует!';
              end;
            end if;
          end;
        end if;
    ...
    if (V_STR_MSG is null) then
      begin
        update WB_REF_WS_AIR_DOW_CR_CODES
        set CR_CODE_NAME=P_CR_CODE_NAME,
            FC_NUMBER=P_FC_NUMBER,
	          CC_NUMBER=P_CC_NUMBER,
    ...
    commit;
  end SP_WB_REF_WS_AIR_DOW_CR_C_UPD;
/
