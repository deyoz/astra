create or replace PROCEDURE SP_WB_REF_GET_AHM_AIRCR_INFO
(cXML_in in clob,
   cXML_out out clob)
AS
...
BEGIN
  ...
  from
  (
    select t1.ID, t2.NAME_ENG_SMALL weight, t4.NAME_ENG_SMALL length, t3.NAME_ENG_SMALL volume, 'l' liguid_volume, t5.NAME_ENG_SMALL fuel_density, t6.NAME_ENG_SMALL moment
  ...
  begin
    cXML_out := '<?xml version="1.0" encoding="utf-8"?><root name="get_ahm_aircraft_info" result="ok">' || cXML_data.getClobVal() || '</root>';
  end;
  else
  begin
    cXML_out := '<?xml version="1.0" encoding="utf-8"?><root name="get_ahm_aircraft_info" result="ok"></root>';
  end;
  end if;

  commit;
END SP_WB_REF_GET_AHM_AIRCR_INFO;
/
