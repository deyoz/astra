create or replace procedure SP_WB_REF_WS_AIR_CMP_T_INS
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
...
V_R_COUNT int:=0;
  begin
   ...
   /*
    ...
    if (V_STR_MSG is null) then
      begin
        P_ID:=SEC_WB_REF_WS_AIR_HLD_CMP_T.nextval();

        insert into WB_REF_WS_AIR_HLD_CMP_T (ID,
	                                             ID_AC,
	                                               ID_WS,
	                                                 ID_BORT,
	                                                   HOLD_ID,
    ...
    commit;
  end SP_WB_REF_WS_AIR_CMP_T_INS;
/
