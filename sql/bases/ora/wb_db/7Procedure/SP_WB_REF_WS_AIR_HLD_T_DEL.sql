    ...
    if V_R_COUNT>0 then
      begin
        if P_LANG='ENG' then
          ...
          begin
            V_STR_MSG:='На значения "Hold Name" имеются ссылки в блоке "Compartments"! Операция запрещена!';
          end;
        end if;
      end;
    end if;
        ...
        if V_R_COUNT>0 then
          begin
            if P_LANG='ENG' then
              ...
              begin
                V_STR_MSG:='На значения "Hold Name" имеются ссылки в блоке "Doors"! Операция запрещена!';
              end;
            end if;
          end;
        end if;
