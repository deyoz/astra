create or replace procedure SP_WB_SET_USER_SETTINGS
(cXML_in in clob,
   cXML_out out clob)
as
P_ELEM_ID number:=-1;
...
begin
  ...
  select replace(cXML_in, '"set_user_settings"', '"get_user_settings"') into cXML_out from dual;
...
<!--
    Ответ.
-->
<root name="set_user_settings" result="ok">
</root>';
  commit;
end SP_WB_SET_USER_SETTINGS;
/
