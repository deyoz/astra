create or replace procedure SP_WB_REF_WS_AIR_MAX_W_DATA
(cXML_in in clob,
   cXML_out out clob)
as
P_ID_AC number:=-1;
          ...
          begin
            SELECT XMLAGG(XMLELEMENT("table_data", xmlattributes(e.id as "id",
                                                                   e.BORT_NUM as "BORT_NUM",
                                                                     e.MAX_RMP_WEIGHT as "MAX_RMP_WEIGHT",
	                                                                     e.MAX_ZF_WEIGHT as "MAX_ZF_WEIGHT",
	                                                                       e.MAX_TO_WEIGHT as "MAX_TO_WEIGHT",
	                                                                         e.MAX_LND_WEIGHT as "MAX_LND_WEIGHT",
            ...
            from (select to_char(rw.id) id,
                           wb.BORT_NUM,
                             to_char(MAX_RMP_WEIGHT) MAX_RMP_WEIGHT,
	                             to_char(MAX_ZF_WEIGHT) MAX_ZF_WEIGHT,
	                               to_char(MAX_TO_WEIGHT) MAX_TO_WEIGHT,
	                                 to_char(MAX_LND_WEIGHT) MAX_LND_WEIGHT,
                  ...
                  from WB_REF_WS_AIR_MAX_WGHT_T rw join WB_REF_AIRCO_WS_BORTS wb
                  on rw.IDN=P_IDN and
                     rw.ID_BORT=wb.id
...
end SP_WB_REF_WS_AIR_MAX_W_DATA;
/
