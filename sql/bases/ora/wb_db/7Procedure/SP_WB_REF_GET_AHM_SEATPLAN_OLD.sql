create or replace PROCEDURE SP_WB_REF_GET_AHM_SEATPLAN_OLD
(cXML_in in clob,
   cXML_out out clob)
AS
cXML_data XMLType; P_ID number:=-1; Units varchar2(100); vID_AC number; vID_WS number; vID_BORT number;
...
BEGIN
  ...
  with tblSeats as
  (
    select Units, tt1.ID_AC, tt1.ID_WS, tt1.IDN, tt1.CABIN_SECTION, tt1.ROW_NUMBER, tt1.LengthOfArm, tt1.Seat_Ident,
      tt1.Codes
      ||
      case
    ...
    from
    (
      select t1.ID_AC, t1.ID_WS, t1.IDN, t1.CABIN_SECTION, t1.ROW_NUMBER, t1.LengthOfArm, t1.Seat_Ident,
      ...
      from
      (
        select t1.ID_AC, t1.ID_WS, t1.IDN, t1.CABIN_SECTION, t1.ROW_NUMBER,
              case
      ...
      group by t1.ID_AC, t1.ID_WS, t1.IDN, t1.CABIN_SECTION, t1.ROW_NUMBER, t1.LengthOfArm, t1.Seat_Ident
    ) tt1
  )
  ...
  commit;
END SP_WB_REF_GET_AHM_SEATPLAN_OLD;
/
