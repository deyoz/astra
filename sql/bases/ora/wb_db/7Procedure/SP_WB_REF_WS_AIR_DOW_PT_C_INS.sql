create or replace procedure SP_WB_REF_WS_AIR_DOW_PT_C_INS
(cXML_in in clob,
   cXML_out out clob)
as
...
V_R_COUNT int:=0;
  begin
    ...
    select extractValue(xmltype(cXML_in),'/root[1]/list[1]/P_PT_CODE_NAME[1]') into P_PT_CODE_NAME from dual;
    ...
    if V_STR_MSG is null then
      begin
        ...
        where PHRASE=P_PT_CODE_NAME;

        if V_R_COUNT>0 then
          begin
            if P_LANG='ENG' then
              begin
                 V_STR_MSG:='Value '||P_PT_CODE_NAME||' is a phrase reserved!';
              end;
            else
              begin
                V_STR_MSG:='Значение '||P_PT_CODE_NAME||' является зарезервированной фразой!';
              end;
            end if;
          end;
        end if;
    ...
    if V_STR_MSG is null then
      begin
        ...
        where id_ac=P_ID_AC and
              ...
              PT_CODE_NAME=P_PT_CODE_NAME;

      if V_R_COUNT>0 then
          begin
            if P_LANG='ENG' then
              begin
                 V_STR_MSG:='Recording with the value of the field "Pantry Code Name" already exists!';
              end;
            else
              begin
                V_STR_MSG:='Запись с таким значением поля "Pantry Code Name" уже существует!';
              end;
            end if;
          end;
        end if;
    ...
    if (V_STR_MSG is null) then
      begin
        V_ID:=SEC_WB_REF_WS_AIR_DOW_PT_CODES.nextval();

        insert into WB_REF_WS_AIR_DOW_PT_CODES (ID,
	                                               ID_AC,
	                                                 ID_WS,
	                                                   ID_BORT,
	                                                     PT_CODE_NAME,
        ...
        if P_BY_DEFAULT=1 then
          begin
            update WB_REF_WS_AIR_DOW_PT_CODES
            set BY_DEFAULT=0
            where id_ac=P_ID_AC and
                  id_ws=P_ID_WS and
                  id_bort=P_ID_BORT and
    ...
    commit;
  end SP_WB_REF_WS_AIR_DOW_PT_C_INS;
/
