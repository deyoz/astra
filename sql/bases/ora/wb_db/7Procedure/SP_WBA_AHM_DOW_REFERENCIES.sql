create or replace PROCEDURE SP_WBA_AHM_DOW_REFERENCIES(cXML_in IN clob, cXML_out OUT CLOB)
AS
-- Блок данных со схемой багажного отеделния
-- Создал: Набатов Т.Е.
cXML_data XMLType; P_ID number:=-1;  P_ID_AC number; P_ID_WS number; P_ID_BORT number; P_ID_SL number; P_IDN number;
REC_COUNT number:=0;
REC_COUNT2 number:=0;
ULD_Weight number := 0;
ULD_Idx number := 0;
...
END SP_WBA_AHM_DOW_REFERENCIES;
/
