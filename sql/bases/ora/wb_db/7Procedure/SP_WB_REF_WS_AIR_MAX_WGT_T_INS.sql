create or replace procedure SP_WB_REF_WS_AIR_MAX_WGT_T_INS
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
...
V_R_COUNT int:=0;
  begin
   ...
   P_MAX_LND_WEIGHT:=to_number(P_MAX_LND_WEIGHT_INT_PART||'.'||P_MAX_LND_WEIGHT_DEC_PART);
    ...
    if (V_STR_MSG is null) then
      begin
        P_ID:=SEC_WB_REF_WS_AIR_MAX_W_T.nextval();

        insert into WB_REF_WS_AIR_MAX_WGHT_T (ID,
	                                              ID_AC,
	                                                ID_WS,
		                                                ID_BORT,
                                                      IDN,
    ...
    commit;
  end SP_WB_REF_WS_AIR_MAX_WGT_T_INS;
/
