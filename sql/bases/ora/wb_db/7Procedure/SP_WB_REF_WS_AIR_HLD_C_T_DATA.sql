create or replace procedure SP_WB_REF_WS_AIR_HLD_C_T_DATA
(cXML_in in clob,
   cXML_out out clob)
as
P_ID_AC number:=-1;
    ...
    if V_R_COUNT>0 then
      begin
        ...
        from (select to_char(h.id) id,
              ...
              from WB_REF_WS_AIR_HLD_CMP_T h join WB_REF_WS_AIR_HLD_HLD_T d
...
end SP_WB_REF_WS_AIR_HLD_C_T_DATA;
/
