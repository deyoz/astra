create or replace procedure SP_WB_REF_WS_AIR_DOW_CR_C_INS
(cXML_in in clob,
   cXML_out out clob)
as
...
V_R_COUNT int:=0;
  begin
    ...
    if (V_STR_MSG is null) then
      begin
        V_ID:=SEC_WB_REF_WS_AIR_DOW_CR_CODES.nextval();

        insert into WB_REF_WS_AIR_DOW_CR_CODES (ID,
	                                               ID_AC,
	                                                 ID_WS,
	                                                   ID_BORT,
	                                                     CR_CODE_NAME,
    ...
    commit;
  end SP_WB_REF_WS_AIR_DOW_CR_C_INS;
/
