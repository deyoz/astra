create or replace procedure SP_WB_SET_SEATING_DETAILS
(cXML_in in clob,
   cXML_out out clob)
as
P_ELEM_ID number:=-1;
...
begin
  ...
  select replace(cXML_in, '"set_seating_details"', '"get_seating_details"') into cXML_out from dual;
...
<!--
    Ответ.
-->
<root name="set_seating_details" result="ok">
</root>';
  commit;
end SP_WB_SET_SEATING_DETAILS;
/
