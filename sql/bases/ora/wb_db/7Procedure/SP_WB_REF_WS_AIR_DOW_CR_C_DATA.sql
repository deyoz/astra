create or replace procedure SP_WB_REF_WS_AIR_DOW_CR_C_DATA
(cXML_in in clob,
   cXML_out out clob)
as
P_ID_AC number:=-1;
...
begin
    ...
    if V_R_COUNT>0 then
      begin
        SELECT XMLAGG(XMLELEMENT("crew_code_list", xmlattributes(e.id as "id",
                                                                   e.CR_CODE_NAME "CR_CODE_NAME",
                                                                     e.FC_NUMBER as "FC_NUMBER",
                                                                       e.CC_NUMBER as "CC_NUMBER",
        ...
        from (select to_char(id) as id,
                       case when CR_CODE_NAME is null then 'NULL' else CR_CODE_NAME end as CR_CODE_NAME,
                         case when FC_NUMBER is null then 'NULL' else to_char(FC_NUMBER) end as FC_NUMBER,
                           case when CC_NUMBER is null then 'NULL' else to_char(CC_NUMBER) end as CC_NUMBER,
              ...
              where id_ac=P_ID_AC and
                    id_ws=P_ID_WS and
                    id_bort=P_ID_BORT
              order by case when CR_CODE_NAME is null then 'NULL' else CR_CODE_NAME end) e
              order by e.CR_CODE_NAME;
...
end SP_WB_REF_WS_AIR_DOW_CR_C_DATA;
/
