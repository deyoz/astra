create or replace PROCEDURE SP_WB_REF_WS_AIR_BO_INFO(cXML_in IN clob,
                                                        cXML_out OUT CLOB)
AS
...
V_R_COUNT number;
  BEGIN
    ...
    from WB_REF_WS_AIR_BAL_OUTPUT_ITEM i join WB_REF_WS_AIR_BAL_OUT_ADV_COL da
    on 1=1 join WB_REF_WS_AIR_BAL_OUT_DET_COL dd
   ...
   --   begin
        ...
        from (select distinct i.id as item_id,
                              ...
                              case when P_LANG='ENG' then i.ACRONYM_ENG else i.ACRONYM_RUS end acronym_name,
  ...
  --  end if;
    ...
    from WB_REF_WS_AIR_BAL_OUTPUT_ADV od
    where od.id=P_ID;
    ...
    if V_R_COUNT>0 then
      begin
        ...
        from (select i.id as item_id,
             ...
             from WB_REF_WS_AIR_BAL_OUTPUT_ITEM i join WB_REF_WS_AIR_BAL_OUT_ADV_COL da
             on 1=1 join WB_REF_WS_AIR_BAL_OUT_DET_COL dd
                on dd.ADV_COL_ID=da.id join WB_REF_WS_AIR_BAL_OUTPUT_DATA od
        ...
        from WB_REF_WS_AIR_BAL_OUTPUT_ADV_I
        where adv_id=P_ID;
