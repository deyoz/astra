create or replace PROCEDURE SP_WB_REF_AC_MES_GET_DATE_INFO(cXML_in IN clob,
                                                             cXML_out OUT CLOB)
AS
P_ADV_ID number:=-1;
P_REMARK clob:='';
...
r_count number:=0;
  BEGIN
    ...
    from WB_REF_AIRCO_AUTO_MES_DET
    ...
    if (r_count)>0 then
      begin
        SELECT XMLAGG(XMLELEMENT("list", xmlattributes(e.MES_ID "mes_id",
                                                         e.u_name "U_NAME",
                                                           e.u_ip "U_IP",
                                                             e.u_host_name "U_HOST_NAME",
                                                               e.date_write "DATE_WRITE"))).getClobVal() into cXML_data
        from (select distinct to_char(mes_id) mes_id,
                              U_NAME,
                              U_IP,
                              U_HOST_NAME,
  ...
  END SP_WB_REF_AC_MES_GET_DATE_INFO;
/
