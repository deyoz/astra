create or replace PROCEDURE SP_WB_REF_GET_LOAD_CONTROL_OLD
(cXML_in in clob,
   cXML_out out clob)
AS
...
vCount number; vCount2 number; vXMLParamIn clob; vXMLParamOut clob; cXML_distribution XMLType; cXML_zfw XMLType; cXML_tow XMLType; cXML_ldw XMLType; cXML_mac XMLType; cXML_itl XMLType;
...
--  end if;
  ...
  if vCount > 0 then
  begin
    -- select EXTRACTVALUE(value(b), '/Start/@Weight'), to_number(EXTRACTVALUE(value(b), '/Start/@Idx'), '99999.9999') DryOperatingIndex,
    ...
    -- from WB_CALCS_XML t1, table(XMLSequence(Extract(xmltype(t1.XML_VALUE), '/root/Start'))) b
    where t1.ELEM_ID = P_ID and t1.DATA_NAME = 'DOWData';
  end;
  end if;
                ...
                XMLELEMENT("seating",
                            ...
                            (
                              SELECT XMLAGG(
                                              XMLELEMENT(
                                                          "list",
                                                          XMLATTRIBUTES(tt1.SEAT_ID "Seat", 0 "Estimated", tt1.cnt "Actual", tt2.cnt_max "Maximum")
                                                        ) order by tt1.SEAT_ID
                                            )
                              from
                              (
                              ...
                              (
                                ...
                                (
                                  ...
                                  where t1.ID_AC = vID_AC and t1.ID_WS = vID_WS and t7.IDN = vID_SL -- and t1.ID_BORT = vID_BORT
                              ...
                              ) tt2 on tt1.SEAT_ID = tt2.CABIN_SECTION
                            )
                          )
                )
...
*/
  ...
  cXML_out := cXML_out || '<list load="BAG" Estimated="0" Actual="' || to_char(vFBag + vCBag + vYBag) || '" Maximum=""/>';
  ...
  (
    ...
    group by tt1.SEAT_ID, tt1.ClassCode
        ...
        inner join (
                      ...
                      where t1.ID_AC = vID_AC and t1.ID_WS = vID_WS and t2.IDN = vID_SL -- and t1.ID_BORT = vID_BORT
  ...
  select (vC_CONST * (vLI_TOW - vK_CONST) / (case when vZFW = 0 then 1 else vZFW end) + vREF_ARM - vLEMAC_LERC) * 100 / vLEN_MAC_RC
  into vMAC_TOW
  from dual;

  select (vC_CONST * (vLI_LAW - vK_CONST) / (case when vZFW = 0 then 1 else vZFW end) + vREF_ARM - vLEMAC_LERC) * 100 / vLEN_MAC_RC
  into vMAC_LAW
  from dual;
...
*/
  ...
  -- commit;
END SP_WB_REF_GET_LOAD_CONTROL_OLD;
/
