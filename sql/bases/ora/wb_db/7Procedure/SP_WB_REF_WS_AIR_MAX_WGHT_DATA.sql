create or replace procedure SP_WB_REF_WS_AIR_MAX_WGHT_DATA
(cXML_in in clob,
   cXML_out out clob)
as
P_ID number:=-1;
    ...
    if V_R_COUNT>0 then
      begin
        select '<adv_data TABLE_NAME="'||i.TABLE_NAME||'" '||
           ...
           INTO cXML_data
           FROM WB_REF_WS_AIR_MAX_WGHT_IDN i
           WHERE i.ID=P_ID;

        cXML_out:=cXML_out||cXML_data;

        select count(u.id) into V_R_COUNT
        from WB_REF_WS_AIR_MAX_WGHT_USE a join WB_REF_WS_AIR_GR_CH_USE_ITEMS u
        on a.idn=P_ID and
           u.id=a.USE_ITEM_ID;
            ...
            from (select ui.ITEM_NAME,
                           case when u.value_1 is NULL
                             ...
                             case when u.value_2 is NULL
                               ...
                               u.USE_ITEM_ID
                  from WB_REF_WS_AIR_MAX_WGHT_USE u join WB_REF_WS_AIR_GR_CH_USE_ITEMS ui
                  on u.idn=P_ID and
                     ui.id=u.USE_ITEM_ID
...
end SP_WB_REF_WS_AIR_MAX_WGHT_DATA;
/
