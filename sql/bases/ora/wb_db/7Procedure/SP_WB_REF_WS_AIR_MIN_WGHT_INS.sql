create or replace procedure SP_WB_REF_WS_AIR_MIN_WGHT_INS
(cXML_in in clob,
   cXML_out out clob)
as
V_IDN number:=-1;
...
V_R_COUNT number:=-1;
  begin
    ...
    if V_STR_MSG is null then
      begin
        V_IDN:=SEC_WB_REF_WS_AIR_MIN_WGHT_IDN.nextval();

        insert into WB_REF_WS_AIR_MIN_WGHT_IDN(ID,
	                                               ID_AC,
	                                                 ID_WS,
		                                                 ID_BORT,
                                                       TABLE_NAME,
    ...
    commit;
  end SP_WB_REF_WS_AIR_MIN_WGHT_INS;
/
