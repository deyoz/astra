create or replace PROCEDURE SP_WB_REF_GET_LOADSHEET_OLD
...
vCount number; vCount2 number; vXMLParamIn clob; vXMLParamOut clob;
...
BEGIN
  ...
  into cVersion_5
  from WB_SCHED t1 inner join WB_REF_WS_AIR_S_L_C_ADV t2 on t1.ID_AC = t2.ID_AC and t1.ID_WS = t2.ID_WS and t1.ID_BORT = t2.ID_BORT and t1.ID_SL = t2.ID
  ...
  from
  (
    select t1.code, sum(nvl(t1.Adult, 0) + nvl(t1.Male, 0) + nvl(t1.Female, 0) + nvl(t1.Child, 0) + nvl(t1.Infant, 0)) pass
    from
    (
      select EXTRACTVALUE(value(b), '/class/@code') code, EXTRACTVALUE(value(b), '/class/@Adult') Adult, EXTRACTVALUE(value(b), '/class/@Male') Male, EXTRACTVALUE(value(b), '/class/@Female') Female,
        EXTRACTVALUE(value(b), '/class/@Child') Child, EXTRACTVALUE(value(b), '/class/@Infant') Infant, EXTRACTVALUE(value(b), '/class/@CabinBaggage') CabinBaggage
...
<root><list>
  ...
  cBlnce_and_Seat_Cnd_36_DOI := rtrim(to_char(nvl(vDOI, 0), 'FM999990.99'), '.');
  cBlnce_and_Seat_Cnd_36_TOFI := rtrim(to_char(nvl(vTakeOffFuelIdx, 0), 'FM999990.99'), '.');
  cBlnce_and_Seat_Cnd_36_LI_ZFW := rtrim(to_char(vLI_ZFW, 'FM999990.99'), '.');
  cBlnce_and_Seat_Cnd_36_LI_TOW := rtrim(to_char(vLI_TOW, 'FM999990.99'), '.');
  cBlnce_and_Seat_Cnd_36_LI_LAW := rtrim(to_char(vLI_LAW, 'FM999990.99'), '.');
  cBlnce_and_Seat_Cnd_36_MAC_ZFW := rtrim(to_char(vMAC_ZFW, 'FM999990.99'), '.');
  cBlnce_and_Seat_Cnd_36_MAC_TOW := rtrim(to_char(vMAC_TOW, 'FM999990.99'), '.');
  cBlnce_and_Seat_Cnd_36_MAC_LAW := rtrim(to_char(vMAC_LAW, 'FM999990.99'), '.');
  ...
  */
  ...
  commit;
END SP_WB_REF_GET_LOADSHEET_OLD;
/
