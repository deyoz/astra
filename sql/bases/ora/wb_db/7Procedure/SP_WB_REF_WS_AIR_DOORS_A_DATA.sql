create or replace procedure SP_WB_REF_WS_AIR_DOORS_A_DATA
(cXML_in in clob,
   cXML_out out clob)
as
P_ID_AC number:=-1;
      ...
      begin
        ...
        if (V_IS_AC>0) and
             (V_IS_WS>0) and
               (V_IS_BORT>0) then
          begin
            insert into WB_REF_WS_AIR_DOORS_A (ID,
	                                               ID_AC,
	                                                 ID_WS,
	                                                   ID_BORT,
	                                                     REMARKS,
	                                                       U_NAME,
	                                                         U_IP,
	                                                           U_HOST_NAME,
	                                                             DATE_WRITE)
            select SEC_WB_REF_WS_AIR_DOORS_A.nextval,
                     P_ID_AC,
                       P_ID_WS,
                         P_ID_BORT,
                           'EMPTY_STRING',
...
end SP_WB_REF_WS_AIR_DOORS_A_DATA;
/
