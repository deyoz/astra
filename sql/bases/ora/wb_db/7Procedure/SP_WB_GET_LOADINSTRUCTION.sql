create or replace PROCEDURE SP_WB_GET_LOADINSTRUCTION
...
CURSOR cCMPs (cP_ID NUMBER) IS
  ...
  (
    ...
    from WB_REF_WS_AIR_HLD_DECK t1 inner join WB_REF_WS_AIR_HLD_HLD_T t2 on t1.DECK_ID = t2.DECK_ID and t1.ID_AC = t2.ID_AC and t1.ID_WS = t2.ID_WS
                                  ...
                                  inner join WB_SCHED t13 on t13.ID = cP_ID and t1.ID_AC = t13.ID_AC and t1.ID_WS = t13.ID_WS and t1.ID_BORT = t13.ID_BORT
                                  join (
...
BEGIN
  ...
  select t1.ID_AC, t1.ID_WS, t1.ID_BORT, t1.ID_SL, t1.NR, to_char(t1.STD, 'DDMONYY', 'nls_date_language=English'), to_char(t1.STD, 'HH24MI'), t1.GATE, 'Stand', 'NIL', substr(t1.NR, 1, 2)
  into vID_AC, vID_WS, vID_BORT, vID_SL, cFlight_3, cDate_8, cTime_9, cGate_6, cStand_7, cJoiningSpecs_12, cAirCoCode
  from WB_SCHED t1
  ...
  (
    ...
    (
      ...
      from WB_REF_WS_AIR_HLD_DECK t1 inner join WB_REF_WS_AIR_HLD_HLD_T t2 on t1.DECK_ID = t2.DECK_ID and t1.ID_AC = t2.ID_AC and t1.ID_WS = t2.ID_WS
                                    ...
                                    left join WB_REF_ULD_IATA t9 on t8.ULD_IATA_ID = t9.ID
                                    left join WB_REF_ULD_TYPES t10 on t9.TYPE_ID = t10.ID
                                    inner join WB_SCHED t13 on t13.ID = P_ID and t1.ID_AC = t13.ID_AC and t1.ID_WS = t13.ID_WS -- and t1.ID_BORT = t13.ID_BORT
                                    inner join WB_SCHED_MRSHR t14 on t13.ID = t14.ELEM_ID
                                    join (
                                            select t1.ELEM_ID, EXTRACTVALUE(value(b), '/row/@TypeOfLoad') TypeOfLoad, EXTRACTVALUE(value(b), '/row/@Position') Position, EXTRACTVALUE(value(b), '/row/@IsBulk') IsBulk,
                                                  EXTRACTVALUE(value(b), '/row/@ULDIATAType') ULDIATAType, EXTRACTVALUE(value(b), '/row/@NetWeight') NetWeight, EXTRACTVALUE(value(b), '/row/@Finalized') Finalized,
