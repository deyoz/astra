create or replace PROCEDURE SP_WB_REF_GET_DOW_DATA_TEMP(cXML_in IN clob,
                                           cXML_out OUT CLOB)
AS
cXML_Data XMLType;
P_ELEM_ID number := -1;
...
BEGIN
  ...
  begin
    ...
    begin
      cXML_out := '<root name="get_dow_data" result="ok">' || cXML_out || '</root>';
    end;
    end if;
  end;
  end if;
  ...
  begin
    cXML_out := '<?xml version="1.0" encoding="utf-8"?><root name="get_dow_data" result="ok"></root>';
  end;
  end if;
END SP_WB_REF_GET_DOW_DATA_TEMP;
/
