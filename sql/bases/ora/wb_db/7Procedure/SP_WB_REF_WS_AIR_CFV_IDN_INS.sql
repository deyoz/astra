create or replace procedure SP_WB_REF_WS_AIR_CFV_IDN_INS
(cXML_in in clob,
   cXML_out out clob)
as
V_IDN number:=-1;
...
V_R_COUNT number:=-1;
  begin
    ...
    if V_STR_MSG is null then
      begin
        V_IDN:=SEC_WB_REF_WS_AIR_CFV_IDN.nextval();

        select count(id) into V_ID
        from WB_REF_WS_AIR_CFV_IDN
        where id_ac=P_ID_AC and
              id_ws=P_ID_WS and
              id_bort=P_ID_BORT;

        insert into WB_REF_WS_AIR_CFV_IDN (ID,
	                                           ID_AC,
	                                             ID_WS,
	                                               ID_BORT,
                                                   TABLE_NAME,
        ...
        insert into WB_REF_WS_AIR_CFV_ADV (ID,
    	                                         ID_AC,
	                                               ID_WS,
                                                 	 ID_BORT,
                                             	       IDN,
	                                                                             ...
	                                                                             U_NAME,
	                                                                               U_IP,
	                                                                                 U_HOST_NAME,
	                                                                                   DATE_WRITE,
                                                                                       DATE_FROM)
        select SEC_WB_REF_WS_AIR_CFV_ADV.nextval,
                 P_ID_AC,
                   P_ID_WS,
                     P_ID_BORT,
                       V_IDN,
    ...
    commit;
  end SP_WB_REF_WS_AIR_CFV_IDN_INS;
/
