create or replace PROCEDURE SP_WB_REF_GET_LOAD_CONTROL
(cXML_in in clob,
   cXML_out out clob)
AS
...
--  end if;
  ...
  begin
    ...
    -- Считаем ULDs
    /*
    select sum(t5.TARE_WEIGHT) NetWeight, sum(t5.TARE_WEIGHT * t4.INDEX_PER_WT_UNIT) Idx
    into vULDWeight, vULDIdx
...
*/
  ...
  -- commit;
END SP_WB_REF_GET_LOAD_CONTROL;
/
