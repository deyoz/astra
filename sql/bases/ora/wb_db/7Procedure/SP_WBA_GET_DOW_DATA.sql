create or replace PROCEDURE SP_WBA_GET_DOW_DATA(cXML_in IN clob,
                                           cXML_out OUT CLOB)
AS
cXML_Data XMLType;
P_ELEM_ID number := -1;
...
BEGIN
  ...
  begin
    ...
    begin
      cXML_out := '<?xml version="1.0" encoding="utf-8"?><root name="get_dow_data" elem_id="' || to_char(P_ELEM_ID) || '" result="ok">' || cXML_out || '</root>'; -- cp866
    end;
    end if;
  end;
  end if;
  ...
  begin
    cXML_out := '<?xml version="1.0" encoding="utf-8"?><root name="get_dow_data" elem_id="' || to_char(P_ELEM_ID) || '" result="ok"></root>'; -- utf-8
  end;
  end if;
END SP_WBA_GET_DOW_DATA;
/
