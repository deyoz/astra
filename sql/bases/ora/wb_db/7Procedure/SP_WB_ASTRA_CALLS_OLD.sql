create or replace PROCEDURE SP_WB_ASTRA_CALLS_OLD(cXML_in IN clob, cXML_out OUT clob)
AS
l_XML clob;
...
BEGIN
  ...
  case sKey
    ...
    when 'get_load_control' then
      begin
        SP_WB_REF_GET_LOAD_CONTROL(l_XML, cXML_out);
      end;
    ...
    when 'get_ahm_airline_uld_types' then
      begin
        SP_WB_REF_GET_ULD_TYPES(l_XML, cXML_out);
      end;
    ...
    when 'get_ahm_airline_info' then
      begin
        SP_WB_REF_GET_AHM_AIRLINE_INFO(l_XML, cXML_out);
      end;
    when 'get_ahm_aircraft_info' then
      begin
        SP_WB_REF_GET_AHM_AIRCR_INFO(l_XML, cXML_out);
      end;
    when 'get_ahm_airline_load_codes' then
      begin
        SP_WB_REF_GET_LOAD_CODES(l_XML, cXML_out);
      end;
    when 'get_ahm_airline_uld_types' then
      begin
        SP_WB_REF_GET_ULD_BORT_TYPES(l_XML, cXML_out);
      end;
    when 'get_ahm_deadload_decks' then
      begin
        SP_WB_REF_GET_AHM_DEADLD_DECKS(l_XML, cXML_out);
      end;
    when 'get_ahm_deadload_doors' then
      begin
        SP_WB_REF_GET_AHM_DEADLD_DOORS(l_XML, cXML_out);
      end;
    when 'get_ahm_passenger_and_baggage_weights' then
      begin
        SP_WB_REF_GET_FLIGHT_CLASSES(l_XML, cXML_out);
      end;
    when 'get_ahm_seatplan' then
      begin
        SP_WB_REF_GET_AHM_SEATPLAN(l_XML, cXML_out);
      end;
    when 'get_deadload_details' then
      begin
        SP_WB_REF_GET_DEAD_DETAILS(l_XML, cXML_out);
      end;
    when 'get_dow_data' then
      begin
        SP_WB_REF_GET_DOW_DATA(l_XML, cXML_out);
      end;
    when 'get_ahm_dow_referencies' then
      begin
        SP_WB_REF_GET_DOW_REF(l_XML, cXML_out);
      end;
    when 'get_flight_stages' then
      begin
        SP_WB_REF_GET_FLIGHT_STAGES(l_XML, cXML_out);
      end;
    when 'get_passengers_details' then
      begin
        SP_WB_REF_GET_PASSENGERS(l_XML, cXML_out);
      end;
    when 'get_seating_details' then
      begin
        SP_WB_REF_GET_SEATING_DETAILS(l_XML, cXML_out);
      end;
    ...
    when 'set_configuration' then
      begin
        SP_WB_SET_CONFIGURATION(l_XML, cXML_out);
      end;
    when 'set_configuration' then
      begin
        SP_WB_SET_CONFIGURATION(l_XML, cXML_out);
    ...
    when 'get_query_set_flight' then
      begin
        SP_WB_GET_SET_FLIGHT(l_XML, cXML_out);
      end;
    when 'get_schedule' then
      begin
        -- SP_WB_GET_SCHEDULE(l_XML, cXML_out);
        SP_WB_REF_GET_SHEDULE(l_XML, cXML_out);
      end;
    ...
    when 'save_loadinstruction' then
      begin
        SP_WB_SAVE_LOADINSTRUCTION(l_XML, cXML_out);
      end;
    ...
    when 'save_loadsheet' then
      begin
        SP_WB_SAVE_LOADSHEET(l_XML, cXML_out);
      end;
