create or replace procedure SP_WB_REF_WS_AIR_DOW_PT_C_DATA
(cXML_in in clob,
   cXML_out out clob)
as
P_ID_AC number:=-1;
...
begin
    ...
    if V_R_COUNT>0 then
      begin
        SELECT XMLAGG(XMLELEMENT("pantry_code_list", xmlattributes(e.id as "id",
                                                                     e.PT_CODE_NAME "PT_CODE_NAME",
        ...
        from (select to_char(id) as id,
                       case when PT_CODE_NAME is null then 'NULL' else PT_CODE_NAME end as PT_CODE_NAME,
              ...
              where id_ac=P_ID_AC and
                    id_ws=P_ID_WS and
                    id_bort=P_ID_BORT
              order by case when PT_CODE_NAME is null then 'NULL' else PT_CODE_NAME end) e
              order by e.PT_CODE_NAME;
...
end SP_WB_REF_WS_AIR_DOW_PT_C_DATA;
/
