create or replace procedure SP_WBA_SET_DOW_DATA
(cXML_in in clob,
   cXML_out out clob)
as
P_ELEM_ID number := -1;
...
begin
  ...
  select t1.ID_AC, t1.ID_WS, t1.ID_BORT-- , t1.ID_SL -- конфигурация - на будущее
  INTO P_ID_AC, P_ID_WS, P_ID_BORT -- , P_ID_SL
  from WB_SHED t1
  where t1.ID = P_ELEM_ID and rownum <= 1;

  select replace(cXML_in, '"set_dow_data"', '"get_dow_data"') into cXML_Temp from dual;

  SP_WBA_RECALC_DOW_DATA(cXML_Temp, cXML_out); -- пересобрали XML
  ...
  select XMLELEMENT("root",
                    XMLATTRIBUTES('get_dow_data' "name", 'ok' "result"),
                    -- CabinConfiguration
                    XMLELEMENT(
                                "CabinConfiguration",
