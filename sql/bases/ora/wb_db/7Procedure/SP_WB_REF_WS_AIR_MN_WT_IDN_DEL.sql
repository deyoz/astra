create or replace procedure SP_WB_REF_WS_AIR_MN_WT_IDN_DEL
(cXML_in in clob,
   cXML_out out clob)
as
...
P_ACTION_NAME varchar2(200):='SP_WB_REF_WS_AIR_MN_WT_IDN_DEL';
...
str_msg clob:=null;
  begin
	                                       ...
	                                       i.TYPE_TO,
		                                       i.U_NAME,
		                                         i.U_IP,
		                                           i.U_HOST_NAME,
		                                             i.DATE_WRITE
    from WB_TEMP_XML_ID_EX t join WB_REF_WS_AIR_MIN_WGHT_IDN i
    on i.id=t.id and
    ...
    where exists(select 1
                 ...
                 where t.id=WB_REF_WS_AIR_MIN_WGHT_IDN.id and
                       t.ACTION_NAME=P_ACTION_NAME and
                       t.ACTION_DATE=P_ACTION_DATE);
    ----------------------------------------------------------------------------
    ----------------------------------------------------------------------------
	                                       ...
	                                       d.OPER_ID_2,
                                             ...
                                             d.U_NAME,
		                               	           d.U_IP,
		                                	           d.U_HOST_NAME,
		                                 	             d.DATE_WRITE
    from WB_TEMP_XML_ID_EX t join WB_REF_WS_AIR_MIN_WGHT_USE d
    on d.idn=t.id and
       t.ACTION_NAME=P_ACTION_NAME and
       t.ACTION_DATE=P_ACTION_DATE;
     ...
     where exists(select 1
                 from WB_TEMP_XML_ID_EX t
                 where t.id=WB_REF_WS_AIR_MIN_WGHT_USE.idn and
                       t.ACTION_NAME=P_ACTION_NAME and
                       t.ACTION_DATE=P_ACTION_DATE);
    ----------------------------------------------------------------------------
    ----------------------------------------------------------------------------
	                                         ...
	                                         d.IS_DEFAULT,
                                               ...
                                               d.U_NAME,
		                               	             d.U_IP,
		                                	             d.U_HOST_NAME,
		                                 	               d.DATE_WRITE
    from WB_TEMP_XML_ID_EX t join WB_REF_WS_AIR_MIN_WGHT_T d
    on d.idn=t.id and
       t.ACTION_NAME=P_ACTION_NAME and
       t.ACTION_DATE=P_ACTION_DATE;
    ...
    where exists(select 1
                 from WB_TEMP_XML_ID_EX t
                 where t.id=WB_REF_WS_AIR_MIN_WGHT_T.idn and
                       t.ACTION_NAME=P_ACTION_NAME and
                       t.ACTION_DATE=P_ACTION_DATE);
    ----------------------------------------------------------------------------
    ----------------------------------------------------------------------------
    ...
    commit;
  end SP_WB_REF_WS_AIR_MN_WT_IDN_DEL;
/
