create or replace procedure SP_WB_REF_WS_AIR_SL_CI_T_DEL
(cXML_in in clob,
   cXML_out out clob)
as
...
P_ACTION_NAME varchar2(200):='SP_WB_REF_WS_AIR_SL_CI_T_DEL';
...
str_msg clob:=null;
  begin
    ...
    if V_STR_MSG is null then
      begin
        ...
        where exists(select 1
                     ...
                     where WB_REF_WS_AIR_SL_CI_T.id=t.id and
                           t.ACTION_NAME=P_ACTION_NAME and
                           t.ACTION_DATE=P_ACTION_DATE);
    ----------------------------------------------------------------------------
    ----------------------------------------------------------------------------
    ...
    commit;
  end SP_WB_REF_WS_AIR_SL_CI_T_DEL;
/
