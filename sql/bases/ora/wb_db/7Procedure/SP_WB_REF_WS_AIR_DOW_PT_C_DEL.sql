create or replace procedure SP_WB_REF_WS_AIR_DOW_PT_C_DEL
(cXML_in in clob,
   cXML_out out clob)
as
P_LANG varchar2(50):='';
...
V_R_COUNT int:=0;
  begin
    ...
    on ws.PANTRY_CODE_ID=t.id;

    if V_R_COUNT>0 then
      begin
        if P_LANG='ENG' then
          begin
            V_STR_MSG:='In the value field "Pantry Code Name" is referenced in the "Weight Configuration Codes" block. The operation is prohibited!';
          end;
        else
          begin
            V_STR_MSG:='На значения поля "Pantry Code Name" имеются ссылки в блоке "Weight Configuration Codes". Операция запрещена!';
          end;
        end if;
      end;
    end if;
    ...
    if (V_STR_MSG is null) then
      begin
        ...
        where exists(select 1
                     ...
                     where WB_REF_WS_AIR_DOW_PT_CODES.id=t.id);
    ...
    commit;
  end SP_WB_REF_WS_AIR_DOW_PT_C_DEL;
/
