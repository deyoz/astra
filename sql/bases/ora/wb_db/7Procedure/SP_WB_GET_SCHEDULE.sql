create or replace PROCEDURE SP_WB_GET_SCHEDULE(cXML_in IN clob, cXML_out OUT CLOB)
AS
cXML_Data XMLType;
P_ELEM_ID number := -1;
REC_COUNT number := 0;
...
ULD_Weight number := 0;
ULD_Idx number := 0;
P_DATE_BEGIN varchar(20) := '';
P_DATE_END varchar(20) := '';
BEGIN
-- Получит даты
select extractValue(xmltype(cXML_in),'/root[1]/@date_begin'), extractValue(xmltype(cXML_in),'/root[1]/@date_end')
into P_DATE_BEGIN, P_DATE_END
from dual;
...
(
  ...
  group by t1.ELEM_ID, EXTRACTVALUE(value(b), '/class/@code')
    ...
    select XMLAGG(
                    XMLELEMENT("elem",
                                XMLATTRIBUTES(
                                              ...
                                              nvl(to_char(t1.S_DTL_1, 'DD.YY.YYYY'), ' ') "DVU",
                                              t1.NR "Flight",
                                              t1.ID_AP_1 "DEP",
                                              t1.ID_WS "ID_WS",
                                  ...
                                  select XMLAGG(
                                                  XMLELEMENT("ARR",
                                                              XMLATTRIBUTES(t1.ID_AP_2 "id",
                                                                            ...
                                                                            nvl((select sum(tt1.CabinBaggage) from tPax tt1 where tt1.elem_id = t1.id and tt1.code = 'F' and tt1.Tr = 0), 0) "CheckedBagClass_F",
                                                                            nvl((select sum(tt1.CabinBaggage) from tPax tt1 where tt1.elem_id = t1.id and tt1.code = 'C' and tt1.Tr = 0), 0) "CheckedBagClass_C",
                                                                            nvl((select sum(tt1.CabinBaggage) from tPax tt1 where tt1.elem_id = t1.id and tt1.code = 'Y' and tt1.Tr = 0), 0) "CheckedBagClass_Y",
                                                                            0 "BookedCargo",
                                                                            0 "BookedMail",
                                                                            0 "LoadedCargo",
                                                                            0 "LoadedMail",
    ...
    where nvl((
                                                    ...
                                                    from WB_CALCS_XML tt1, table(XMLSequence(Extract(xmltype(tt1.XML_VALUE), '/root/CabinConfiguration'))) b
                                                    where tt1.ELEM_ID = t1.ID and tt1.DATA_NAME = 'DOWData' and rownum <= 1
                                                  ), ' ') != ' '; -- (t1.S_DTL_1 >= to_date(P_DATE_BEGIN, 'DD.MM.YYYY') - 10) and (t1.S_DTL_1 < to_date(P_DATE_END, 'DD.MM.YYYY') + 1);

    if cXML_Data is not null then
      cXML_out := cXML_Data.GetCLOBVal();
      -- dbms_output.put_line('Output Length: ' || length(cXML_out));
    end if;
    ...
    begin
      cXML_out := concat(
                          concat(
                                  to_clob('<?xml version="1.0"  encoding="utf-8"?><root name="get_schedule" result="ok">'),
                                  cXML_out
                                ), to_clob('</root> ')
                        );
    end;
    ...
    begin
      cXML_out := '<?xml version="1.0"  encoding="utf-8"?><root name="get_schedule" result="ok"/>';
    end;
    end if;
/*
