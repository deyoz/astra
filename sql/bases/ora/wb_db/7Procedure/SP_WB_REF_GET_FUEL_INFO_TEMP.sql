create or replace PROCEDURE SP_WB_REF_GET_FUEL_INFO_temp
(cXML_in in clob,
   cXML_out out clob)
AS
cXML_data XMLType;
...
BEGIN
  ...
  if REC_COUNT > 0 then
    begin
      ...
      begin
        select to_number(extractValue(xmltype(cXML_in),'/root[1]/fuel_info[1]/@block_fuel'))
        into vBlockFuel
        from dual;

        select to_number(extractValue(xmltype(cXML_in),'/root[1]/fuel_info[1]/@taxi_fuel'))
        into vTaxiFuel
        from dual;

        select to_number(extractValue(xmltype(cXML_in),'/root[1]/fuel_info[1]/@trip_fuel'))
        into vTripFuel
        from dual;
      end;
      else
...
*/
  ...
  (
    select t1.ID_AC, t1.ID_WS, t1.PROC_NAME ProcName, ' ' ProcType, ' ' fuel_distrib, to_char(t1.DENSITY, 'FM990.999') density, t4.NAME_RUS_SMALL density_units, vBlockFuel block_fuel, vTaxiFuel taxi_fuel, -- 0d90
        vTripFuel trip_fuel, 5000 ballast_fuel, 1000 additional_fuel, min(t2.WEIGHT) min_fuel, max(t2.WEIGHT) max_fuel, to_char(max(t2.INDEX_UNIT), 'FM99990.999') index_unit,
  ...
  commit;
END SP_WB_REF_GET_FUEL_INFO_temp;
/
