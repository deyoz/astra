...
BEGIN
  ...
  IF fmt_prefix IS NOT NULL THEN
    ...
    IF 'ЦН' IN (vcountry_regul_control) AND fmt_prefix='EDI' THEN vedi_addr:='CNADAPIS:ZZ'; END IF;
    IF 'ЦН' IN (vcountry_regul_control) AND fmt_prefix='IAPI' THEN vedi_addr:='NIAC'; END IF;
    IF 'ИН' IN (vcountry_regul_control) THEN vedi_addr:='NZCS'; END IF;
    IF 'ЮС' IN (vcountry_regul_control) THEN vedi_addr:='USCSAPIS:ZZ'; END IF;
    IF 'ГБ' IN (vcountry_regul_control) THEN vedi_addr:='UKBAOP:ZZ'; END IF;
    ...
    IF vedi_own_addr IS NULL THEN
      BEGIN
        ...
        IF 'ЦН' IN (vcountry_regul_control) AND fmt_prefix='IAPI' THEN vedi_own_addr:=vaddr_part2; END IF;
      EXCEPTION
        WHEN OTHERS THEN
          info:=adm.get_cache_info('APIS_SETS');
