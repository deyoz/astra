...
[
  ...
  [  --with-eticklib=DIR       enable support for "eticklib" (default $(HOME)/work/eticklib)],
  [
  ...
  if test "x$ETICKLIB" = "x"; then
  	for d in  $HOME/eticklib ../eticklib ../../eticklib ; do
  	if test -d $d; then
  		ETICKLIB=`(cd $d; pwd)`
  		break
  	fi
  ...
  eticklib_ldflags="-L $ETICKLIB/lib"
  eticklib_lib="$ETICKLIB/lib/libetick.a"
