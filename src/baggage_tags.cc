...
//!!! все что ниже потом удалить

bool useNewTagGeneratingAlgo(int point_id)
{
  TQuery Qry(&OraSession);
  Qry.Clear();
  Qry.SQLText=
    "SELECT new_tag_generating_algo.airline "
    "FROM points, new_tag_generating_algo "
    "WHERE points.airline=new_tag_generating_algo.airline AND "
    "      points.point_id=:point_id "
    "UNION "
    "SELECT new_tag_generating_algo.airline "
    "FROM new_tag_generating_algo "
...
/*
int useDeferred(int argc, char **argv)
{
  if (argc<3)
  {
    printf("wrong params count %d\n", argc);
    return 1;
  }

  TGeneratedTags t;

  t.useDeferred(ToInt(argv[1]), ToInt(argv[2]));
  t.trace("after useDeferred");

  return 0;
}

int generateAndDefer(int argc, char **argv)
{
  if (argc<3)
  {
    printf("wrong params count %d\n", argc);
  ...
  t.generate(ToInt(argv[1]), ToInt(argv[2]));
  t.trace("after generate");
  t.defer();
  t.trace("after defer");
...
int generateUsingDeferred(int argc, char **argv)
{
  if (argc<3)
  {
    printf("wrong params count %d\n", argc);
  ...
  t.generateUsingDeferred(ToInt(argv[1]), ToInt(argv[2]));
  t.trace("after generateUsingDeferred");

  return 0;
}

int cleanOldRecords(int argc, char **argv)
{
  if (argc<2)
  {
    printf("wrong params count %d\n", argc);
  ...
  TGeneratedTags::cleanOldRecords(ToInt(argv[1]));
