      ...
      for(std::set<THandler>::const_iterator h=getHandlers().begin();
        ...
        if (h->getType()==THandler::BagMessage)
        {
          BMHandlers[h->getCode()]=BagmessagePtr(new Bagmessage(io,
                                                                ...
                                                                9999999,
                                                                9999999));  //!!!vlad "" ß†¨•≠®‚Ï ≠† msg_handlers.name_lat
        };
    }
...
int main_msg_handler_tcl(int supervisorSocket, int argc, char *argv[])
{
  try
  {
    sleep(2); //!!!vlad Ì‚Æ ≠•Æ°ÂÆ§®¨Æ Á‚Æ°Î cores ≠• ß†°®¢†´® §®·™Æ¢Æ• Ø‡Æ·‚‡†≠·‚¢Æ
...
int test_msg_queue(int argc,char **argv)
{
  ...
  printf("%zu\n", LevenshteinDistance("ÄêÖëíÄçí", "ÑÄÉÖëíÄç"));
  printf("%zu\n", LevenshteinDistance("ÑÄÉÖëíÄç", "ÄêÖëíÄçí"));
...
////  AstraMessages::TQueue::put(setDetails, "", "ùíé íÖãé íÖãÖÉêÄååõ");
////  OraSession.Commit();
