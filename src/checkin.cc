...
bool CheckInInterface::SavePax(xmlNodePtr reqNode, xmlNodePtr ediResNode, xmlNodePtr resNode)
{
  ...
  set<int> tckin_ids;
  bool result=true;
  if (SavePax(reqNode, ediResNode, first_grp_id, ChangeStatusInfo, tckin_id))
  {
    ...
    try
    {
        ...
        for(xmlNodePtr crew_group = crew_groups->children; crew_group != NULL; crew_group = crew_group->next) {
                ...
                for (xmlNodePtr document = pers_data->children; document != NULL; document = document->next) {
                    ...
                    if (strcmp((const char*)document->name,"DOCS") == 0) {
                        ...
                        if (!NodeIsNULLFast("BIRTH_DATE",doc, true))
                            paxForCkin.apis.doc.birth_date = WebSearch::date_fromXML(NodeAsStringFast("BIRTH_DATE",doc,""));
                        ...
                        if (!NodeIsNULLFast("EXPIRY_DATE",doc, true))
                            paxForCkin.apis.doc.expiry_date = WebSearch::date_fromXML(NodeAsStringFast("EXPIRY_DATE",doc,""));
                        ...
                        if (!NodeIsNULLFast("ISSUE_DATE",doc,true))
                            paxForCkin.apis.doco.issue_date=WebSearch::date_fromXML(NodeAsStringFast("ISSUE_DATE",doc,""));
                        if (!NodeIsNULLFast("EXPIRY_DATE",doc,true))
                            paxForCkin.apis.doco.expiry_date=WebSearch::date_fromXML(NodeAsStringFast("EXPIRY_DATE",doc,""));
            ...
            if (emulCkinDoc.docPtr()!=NULL) //регистрация новой группы
            {
              ...
              SavePax(emulReqNode, NULL, first_grp_id, ChangeStatusInfo, tckin_id);
            }
        }
