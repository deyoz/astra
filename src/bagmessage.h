...
class BMConnection
{
  ...
  // Настройки, прочитанные извне при запуске
    ...
    int heartBeatTimeout;       // Время ожидания входящих сообщений. Если за это время к нам ничего не пришло - объявляем тревогу
    int loginTimeout;           // Время ожидания ответа на запрос логина. Если за это время не пришел ответ - шлем логин заново
    int ackTimeout;             // Время ожидания ответа на сообщение ACK_MSG. Если за это время не пришел ответ - шлем заново
    int warmUpTimeout;          // Нормативное время разгона от первой попытки установки связи до получения подтверждения логина.
                                // Если превышено - переключаемся на резервный канал
  // Рабочие переменные
  // Статусы
  ...
  // Связь и протокольные дела
    ...
    time_t channelStart;                  // Время начала попыток установки связи по текущему каналу. Если после этого за интервал
                                          // warmUpTimeout не дошли до получения LOGIN_ACCEPT - переключаемся на другой канал
  // Прием
  ...
  // Передача
    ...
    boost::posix_time::ptime waitForAckTime; // Когда послали это сообщение
