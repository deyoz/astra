...
void SynchMPSEMD( int grp_id, const NotifyPushEvent &p )
{
  ...
  for ( auto& item : prices ) {
    ...
    for ( const auto& svc : svcs ) {
      ...
      for ( const auto& mps_svc : mps_svcs ) {
        ...
        if ( mps_svc.rfisc == item.first.RFISC &&
          ...
          if ( !item.second.findSVC( svc.first, f ) ) {
            throw EXCEPTIONS::Exception( "svc %s not found at TPriceRFISCList", svc.first.svcId.c_str() );
          }
      ...
      if ( !prFind ) {
        throw EXCEPTIONS::Exception( "svc %s not found at MPS notify answer", svc.first.svcId.c_str() );
      }
    }
  }
