...
/*  if ( next != this->dests.items.end() && next->events.isFlag( dmSetUnCancel ) )
  ...
  if ( next != this->dests.end() && this->key.status != tdDelete && this->key.pr_reg &&
    ...
    tst();
    if ( this->key.status == tdInsert ||
         this->key.stations != trip.key.stations ) {
...
/*    if ( tripInfo.flt_no != NoExists )
  ...
  if ( this->events.isFlag( teChangeStations ) ) {
    tst();
    this->key.stations.toDB( "points", this->key.point_id, tstations::dbRewriteAll );
  }
}
...
{
#ifdef ENABLE_ORACLE
  // НЕ РАЗОБРАЛСЯ БЫСТРО С ЗАПРОСОМ - ИГНОРИРУЮ ДЛЯ ДЕМО
  TQuery Qry(&OraSession);
  Qry.SQLText =
...
void TFlights::GetByPointIdTlg(const int point_id_tlg, const TFlightType flightType)
{
  ...
  for(const PointId_t& id : point_ids_spp) Get(id.get(), flightType);
}
