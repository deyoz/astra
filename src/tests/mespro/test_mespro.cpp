...
struct MPCryptParams
{
  ...
  std::string SrvPKey; // Секретный ключ сервера
  std::string SrvPKeyPass; // Пароль к секретному ключу сервера
  std::string ServerCert; // Сертификат сервера

  std::string ClPKey; // Секретный ключ клиента
  std::string ClPKeyPass; // Пароль к секретному ключу клиента
  std::string ClientCert; // Сертификат клиента
};
...
void getMesProCryptParams(int *error, MPCryptParams &params)
{
    ...
    GetServerCertificate( params.CA, params.SrvPKey, params.ServerCert );
    ...
    int pkcs_id = 0;
    bool pr_exists = GetClientCertificate( 8495324, true, desk, params.ClientCert, pkcs_id );
