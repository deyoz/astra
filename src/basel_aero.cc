...
"FROM arx_pax_grp  "
"INNER JOIN arx_pax  ON arx_pax_grp.part_key = arx_pax.part_key AND "
...
"ORDER BY arx_pax.reg_no NULLS LAST, arx_pax.seats DESC NULLS LAST",
    ...
    while(!cur.fen()) {
        ...
        if (stat.pax_id != NoExists)
        {
          ...
          stat.viewStatus = std::string(refuse.empty() ? (pr_brd==0?"зарегистрирован":"прошел посадку")
                                                       : "разрегистрирован").substr(0,30);
