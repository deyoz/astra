...
void PrepRegInterface::readTripCounters( int point_id, xmlNodePtr dataNode )
{
  ...
  Qry.SQLText = "SELECT -100 AS num, "
                       "'Всего' AS firstcol, "
                        ...
                        "SUM(avail) AS avail, "
                        "SUM(prot) AS prot "
           "FROM (SELECT MAX(cfg) AS cfg, "
                        "SUM(crs_ok) AS crs_ok, "
                        "SUM(crs_tranzit) AS crs_tranzit, "
                        "MAX(block) AS block, "
                        "CASE :cfg_exists WHEN 0 THEN CAST(NULL AS SMALLINT) ELSE GREATEST(MAX(avail),0) END AS avail, "
                        "MAX(prot) AS prot "
                "FROM counters2 c "
             "INNER JOIN classes "
                     "ON c.class = classes.code "
              "LEFT JOIN trip_classes "
                     "ON c.point_dep = trip_classes.point_id "
                    "AND c.class = trip_classes.class "
                  "WHERE c.point_dep = :point_id "
                     "AND (trip_classes.point_id IS NOT NULL "
                      "OR :cfg_exists = 0 "
                     "AND :free_seating <> 0) "
               "GROUP BY classes.priority, c.class) a "
           "UNION "
                 "SELECT classes.priority - 10, "
                        ...
                        "SUM(crs_tranzit), "
                        "MAX(block), "
                        "CASE :cfg_exists WHEN 0 THEN CAST(NULL AS SMALLINT) ELSE GREATEST(MAX(avail), 0) END, "
                        "MAX(prot) "
                   "FROM counters2 c "
             "INNER JOIN classes "
                     "ON c.class = classes.code "
           ...
           "UNION "
                 "SELECT points.point_num + 100, "
                        ...
                        "SUM(crs_tranzit), "
                        "SUM(block), "
                        "CASE :cfg_exists WHEN 0 THEN CAST(NULL AS SMALLINT) ELSE GREATEST(SUM(avail), 0) END, "
                        "SUM(prot) "
                   "FROM counters2 c "
             "INNER JOIN points "
                     "ON c.point_arv = points.point_id "
              "LEFT JOIN trip_classes "
                     "ON c.point_dep = trip_classes.point_id "
                    "AND c.class = trip_classes.class "
                  "WHERE c.point_dep = :point_id "
           ...
           "UNION "
                "(SELECT 1 AS num, "
                       "'JMP' AS firstcol, "
                        "jmp_cfg AS cfg, "
                        "CAST(NULL AS SMALLINT) AS resa, "
                        "CAST(NULL AS SMALLINT) AS tranzit, "
                        "CAST(NULL AS SMALLINT) AS block, "
                        "jmp_nooccupy AS avail, "
                        "CAST(NULL AS SMALLINT) AS prot "
                   "FROM counters2 c "
             "INNER JOIN trip_sets "
                     "ON c.point_dep = trip_sets.point_id "
                  "WHERE c.point_dep = :point_id "
                    "AND trip_sets.use_jmp <> 0 "
