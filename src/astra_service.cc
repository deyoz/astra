...
bool createFidsDataFiles( int point_id, const std::string &point_addr, TFileDatas &fds )    //point_addr=BETADC
{
  ...
  reqInfo->Initialize(airp); //!!!djek это ошибка! Надо передавать код города
...
START_TEST(check_file_set_update)
{
  make_db_curs(
    ...
    "values ('СОЧ', 'THE_CODE', 'DIR/', 'SOMENAME', 1, 0)",
    PgOra::getRWSession("FILE_SETS")).exec();

  fail_unless(update_file_sets("THE_CODE", "СОЧ", 10) == 1);
  fail_unless(update_file_sets("THE_CODE", "СОЧ", 10) == 0);
  fail_unless(update_file_sets("THE_CODE", "СОЧ", 0) == 1);
  fail_unless(update_file_sets("THE_CODE2", "СОЧ", 0) == 0);
}
END_TEST;
