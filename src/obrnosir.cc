...
const
  struct {
    const char *name;
    int (*p)(int argc,char **argv);     //Ø‡ÆÊ•§„‡† Æ°‡†°Æ‚™®
    void (*h)(const char *name);                    //ØÆ¨ÆÈÏ
              ...
              TDateTime &old_local, TDateTime &new_local, const std::string &region )
{
   ...
   local_date_time ltime( vtime, tz ); /* ÆØ‡•§•´Ô•¨ ‚•™„È•• ¢‡•¨Ô ´Æ™†´Ï≠Æ• */
   if ( ltime.is_dst() ) { // ´•‚Æ
...
void getPeriods( string filter_tz_region, vector<P> &periods ) {
  ...
  local_date_time ld( utcd, tz ); /* ÆØ‡•§•´Ô•¨ ‚•™„È•• ¢‡•¨Ô ´Æ™†´Ï≠Æ• */
  bool summer = true;
  /* „·‚†≠†¢´®¢†•¨ Ø•‡¢Î© £Æ§ ® Ø‡®ß≠†™ Ø•‡®Æ§† */
  for ( int i=0; i<SEASON_PRIOR_PERIOD; i++ ) {
    if ( tz->has_dst() ) {  // •·´® •·‚Ï Ø•‡•ÂÆ§ ≠† ß®¨≠••/´•‚≠•• ‡†·Ø®·†≠®•
        if ( i == 0 ) {
...
int seasons_dst_format(int argc,char **argv)
{
  ...
  VQry.SQLText = "SELECT move_id FROM routes where move_id=:move_id AND airline='7ä' AND rownum<2";
  ...
  while ( !Qry.Eof ) {
    ...
    int diff = (int)trunc_f - (int)trunc_pf; // Ø•‡•ÂÆ§ ≠† ß®¨„
