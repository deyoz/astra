...
void getFlightTuneCompRef( int point_id, bool use_filter, const string &trip_airline, vector<TShowComps> &comps )
{
  ...
  Qry.SQLText =
    "SELECT craft, "
           "bort, "
           "crc_comp, "
           "COALESCE(SUM(CASE class WHEN 'П' THEN 1 ELSE 0 END), 0) as f, "
           "COALESCE(SUM(CASE class WHEN 'Б' THEN 1 ELSE 0 END), 0) as c, "
           "COALESCE(SUM(CASE class WHEN 'Э' THEN 1 ELSE 0 END), 0) as y "
      "FROM points "
