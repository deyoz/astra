...
namespace PRL_SPACE {
    ...
    void TInfants::get(TypeB::TDetailCreateInfo &info)
    {
        ...
        if(!items.empty()) {
            DB::TQuery QryAdult(PgOra::getROSession({"PAX_GRP","PAX"}), STDLOG);
            string SQLText =
                "SELECT pax.grp_id, "
                "       pax.pax_id, "
                "       pax.reg_no, "
            ...
            if((PRLOptions and PRLOptions->pax_state == "CKIN") or info.get_tlg_type() == "LCI")
                ...
                SQLText += "    pax.pr_brd = 1 ";
            QryAdult.SQLText = SQLText;
            QryAdult.CreateVariable("point_id", otInteger, info.point_id);
            QryAdult.Execute();
            vector<TAdultsItem> adults;
            if(!QryAdult.Eof) {
                int col_grp_id = QryAdult.FieldIndex("grp_id");
                int col_pax_id = QryAdult.FieldIndex("pax_id");
                int col_reg_no = QryAdult.FieldIndex("reg_no");
                int col_surname = QryAdult.FieldIndex("surname");
                for(; !QryAdult.Eof; QryAdult.Next()) {
                    TAdultsItem item;
                    item.grp_id = QryAdult.FieldAsInteger(col_grp_id);
                    item.pax_id = QryAdult.FieldAsInteger(col_pax_id);
                    item.reg_no = QryAdult.FieldAsInteger(col_reg_no);
                    item.surname = QryAdult.FieldAsString(col_surname);
    ...
    void TPRLDest::GetPaxList(TypeB::TDetailCreateInfo &info, vector<TTlgCompLayer> &complayers)
    {
        ...
        SQLText +=
            ...
            "    LEFT OUTER JOIN crs_pnr ON crs_pax.pnr_id = crs_pnr.pnr_id AND crs_pnr.system = 'CRS' "
...
void TFTLBody::get(TypeB::TDetailCreateInfo &info)
{
    ...
    Qry.SQLText =
        ...
        "    pax.name, "
        "    COALESCE(COALESCE(pax.cabin_subclass, pax.subclass), COALESCE(pax.cabin_class, pax_grp.class)) subclass "
        "FROM (pax JOIN pax_grp ON pax_grp.grp_id = pax.grp_id) "
        "JOIN classes ON COALESCE(pax.cabin_class, pax_grp.class) = classes.code "
        "LEFT OUTER JOIN (crs_pax "
...
void TTPLDest::GetPaxList(TypeB::TDetailCreateInfo &info, vector<TTlgCompLayer> &complayers)
{
    DB::TCachedQuery Qry(PgOra::getROSession({"PAX","PAX_GRP","CRS_PNR","CRS_PAX","CLS_GRP"}), // system.transliter
            ...
            "   pax.bag_pool_num "
            "FROM (pax JOIN pax_grp ON pax_grp.grp_id = pax.grp_id) "
            "LEFT OUTER JOIN cls_grp ON COALESCE(pax.cabin_class_grp, pax_grp.class_grp) = cls_grp.id "
            "                           AND cls_grp.code = :class "
            "LEFT OUTER JOIN (crs_pax "
            "    LEFT OUTER JOIN crs_pnr ON crs_pax.pnr_id = crs_pnr.pnr_id AND crs_pnr.system = 'CRS' "
            ") ON pax.pax_id = crs_pax.pax_id AND crs_pax.pr_del = 0 "
...
void TETLDest::GetPaxList(TypeB::TDetailCreateInfo &info,vector<TTlgCompLayer> &complayers)
{
    ...
    if(not(ETLOptions and ETLOptions->rbd)) {
        SQLText += "LEFT OUTER JOIN cls_grp ON COALESCE(pax.cabin_class_grp, pax_grp.class_grp) = cls_grp.id "
                   "                           AND cls_grp.code = :class ";
    }
    SQLText +=
        "LEFT OUTER JOIN (crs_pax "
        ...
        for(const auto &point_arv: route) {
            ...
            for(const auto &pax: pax_list) {
                if(pax->grp().point_arv == point_arv.point_id) {
                    ...
                    item.c += pax->cl() == "Б" and pax->seats();
                    item.y += pax->cl() == "Э" and pax->seats();
...
void TMVTABody::get(TypeB::TDetailCreateInfo &info)
{
    ...
    Qry.SQLText =
        ...
        "ON points.point_id = pax.point_arv "
        "WHERE first_point=:first_point AND point_num>:point_num AND pr_del=0 "
        "ORDER BY point_num ";
    Qry.CreateVariable("point_id", otInteger, info.point_id);
    Qry.CreateVariable("first_point", otInteger, info.pr_tranzit ? info.first_point : info.point_id);
        ...
        vector<string> &si)
{
    for(TLCIPaxTotalsItem::TClsBag::const_iterator
        ...
        for(TLCIPaxTotalsItem::TCategoryBag::const_iterator
                ...
                iCat++) {
            if(iCat->second.first) { // Кол-во багажа не нулевое
                ostringstream result;
                result
...
// Багаж с пустым классом (несопр и багаж экипажа)
struct TEmptyClsBag {
...
const string BCAT_BF = "BF"; // первый
const string BCAT_BC = "BC"; // бизнес
const string BCAT_BY = "BY"; // эконом
const string BCAT_BZ = "BZ"; // багаж доп. экипажа
const string BCAT_BX = "BX"; // несопр. багаж
const string BCAT_DT = "DT"; // багаж экипажа
        ...
        if(isExtraCrew(pax->crew_type)) {
            ...
            if(is_trfer) {
                ...
                bool internal_flight =
                    ((const TCitiesRow&)basecities.get_row("code", ((const TAirpsRow&)baseairps.get_row("code", airp_arv)).city)).country == "РФ";
                internal_flight = internal_flight and
                    ((const TCitiesRow&)basecities.get_row("code", ((const TAirpsRow&)baseairps.get_row("code", trfer_airp_arv)).city)).country == "РФ";
...
int FWD(TypeB::TDetailCreateInfo &info)
{
    ...
    try
    {
        ...
        if(UCMHeadingInfo->tlg_cat == TypeB::tcLOADSHEET)
            heading
                << UCMHeadingInfo->MsgId << std::endl
                << UCMHeadingInfo->flt_info.src << std::endl;
        else
...
struct TCKINPaxInfo {
    ...
    public:
        ...
        TCKINPaxInfo(TInfants *ainfants):
          Qry(PgOra::getROSession({"PAX","PAX_GRP"}), STDLOG),
            col_pax_id(NoExists),
            col_surname(NoExists),
            col_name(NoExists),
            col_seats(NoExists),
...
void TPFSInfo::get(int point_id)
{
    ...
    Qry.SQLText =
        ...
        "    crs_pax.pnr_id  "
        "FROM pax_grp "
        "JOIN (pax LEFT OUTER JOIN crs_pax "
        "          ON pax.pax_id = crs_pax.pax_id AND crs_pax.pr_del = 0 "
        ") ON pax_grp.grp_id = pax.grp_id "
        "WHERE  "
    ...
    DB::TQuery QryTlgBind(PgOra::getROSession({"TLG_BINDING","CRS_PNR","CRS_PAX","PAX","PAX_GRP"}), STDLOG);
    QryTlgBind.SQLText =
        "SELECT  "
        ...
        "    ) ON crs_pax.pax_id = pax.pax_id AND pax.refuse IS NULL "
        ") ON crs_pnr.pnr_id = crs_pax.pnr_id "
        "WHERE  "
        "    tlg_binding.point_id_spp = :point_id AND  "
        "    crs_pnr.system = 'CRS' AND "
        "    crs_pax.pr_del = 0 AND "
        "    COALESCE(pax.seats, crs_pax.seats) > 0";
    QryTlgBind.CreateVariable("point_id", otInteger, point_id);
    QryTlgBind.Execute();
    if(!QryTlgBind.Eof) {
        int col_pnl_pax_id = QryTlgBind.FieldIndex("pnl_pax_id");
        int col_pnl_point_id = QryTlgBind.FieldIndex("pnl_point_id");
        for(; !QryTlgBind.Eof; QryTlgBind.Next()) {
            int pnl_pax_id = QryTlgBind.FieldAsInteger(col_pnl_pax_id);
            int pnl_point_id = NoExists;
            if(!QryTlgBind.FieldIsNULL(col_pnl_point_id))
                pnl_point_id = QryTlgBind.FieldAsInteger(col_pnl_point_id);
...
void TRBD::get(TypeB::TDetailCreateInfo &info)
{
    ...
    if(PRLOptions and PRLOptions->rbd) {
        ...
        DB::TCachedQuery Step3Qry(
                PgOra::getROSession({"CRS_RBD", "TLG_BINDING"}),
                "SELECT "
                "  crs_rbd.point_id, "
                "  crs_rbd.sender, "
                "  (CASE WHEN crs_rbd.point_id = :point_id_tlg_mark THEN 2 ELSE 0 END "
                "   + CASE WHEN crs_rbd.sender = :sender THEN 1 ELSE 0 END) AS priority "
        ...
        if(not Step3Qry.get().Eof) {
            ...
            QryParams
                ...
                << QParam("sender", otString, crs_rbd_sender);
            DB::TCachedQuery Step4Qry(
                    PgOra::getROSession("CRS_RBD"),
...
namespace CKIN_REPORT {
    ...
    void TReportData::get(int apoint_id)
    {
        ...
        for(TTripRoute::iterator iRoute = trip_route.begin(); iRoute != trip_route.end(); iRoute++, idx++) {
            // booking info
            route[idx][iRoute->airp]["П"].booking = crs_pax_list.items[iRoute->airp]["П"];
            route[idx][iRoute->airp]["Б"].booking = crs_pax_list.items[iRoute->airp]["Б"];
...
void TelegramInterface::kuf_file(XMLRequestCtxt *ctxt, xmlNodePtr reqNode, xmlNodePtr resNode)
{
    try {
        ...
        FltOperFilter filter(AirlineCode_t("ЮТ"),
