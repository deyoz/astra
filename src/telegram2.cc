...
namespace PRL_SPACE {
    ...
    void TCOMStats::get(TypeB::TDetailCreateInfo &info)
    {
        ...
        Qry.SQLText =
            ...
            "SELECT "
            "   pax_grp.point_arv, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class), 'П', DECODE(seats,0,0,1), 0)) f, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class), 'Б', DECODE(seats,0,0,1), 0)) c, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class), 'Э', DECODE(seats,0,0,1), 0)) y, "
            "   SUM(DECODE(pax.pers_type, 'ВЗ', 1, 0)) adult, "
            "   SUM(DECODE(pax.pers_type, 'РБ', 1, 0)) child, "
            "   SUM(DECODE(pax.pers_type, 'РМ', 1, 0)) baby, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||pax.pers_type, 'ПРБ', 1, 0)) f_child, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||pax.pers_type, 'ПРМ', 1, 0)) f_baby, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||pax.pers_type, 'БРБ', 1, 0)) c_child, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||pax.pers_type, 'БРМ', 1, 0)) c_baby, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||pax.pers_type, 'ЭРБ', 1, 0)) y_child, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||pax.pers_type, 'ЭРМ', 1, 0)) y_baby, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class), 'П', decode(SIGN(1-pax.seats), -1, 1, 0), 0)) f_add_pax, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class), 'Б', decode(SIGN(1-pax.seats), -1, 1, 0), 0)) c_add_pax, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class), 'Э', decode(SIGN(1-pax.seats), -1, 1, 0), 0)) y_add_pax "
            "FROM "
            "   pax, pax_grp "
            "WHERE "
            "   pax_grp.point_dep = :point_id AND "
            ...
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||bag2.pr_cabin, 'П0', weight, 0)) f_bag_weight, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||bag2.pr_cabin, 'П1', weight, 0)) f_rk_weight, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||bag2.pr_cabin, 'Б0', weight, 0)) c_bag_weight, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||bag2.pr_cabin, 'Б1', weight, 0)) c_rk_weight, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||bag2.pr_cabin, 'Э0', weight, 0)) y_bag_weight, "
            "   SUM(DECODE(nvl(pax.cabin_class, pax_grp.class)||bag2.pr_cabin, 'Э1', weight, 0)) y_rk_weight "
            "FROM "
            "   pax_grp, bag2, pax "
            "WHERE "
            "   pax_grp.grp_id = pax.grp_id(+) and "
...
void TLDMDests::get(TypeB::TDetailCreateInfo &info)
{
    ...
    Qry.SQLText =
        ...
        "             SUM(DECODE(nvl(pax.cabin_class, pax_grp.class),'П',DECODE(seats,0,0,1),0)) AS f, "
        "             SUM(DECODE(nvl(pax.cabin_class, pax_grp.class),'Б',DECODE(seats,0,0,1),0)) AS c, "
        "             SUM(DECODE(nvl(pax.cabin_class, pax_grp.class),'Э',DECODE(seats,0,0,1),0)) AS y, "
