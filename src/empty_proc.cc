    ...
    for ( int itime_type=0; itime_type<3; itime_type++ ) {
      for ( int icond=0; icond<=11; icond++ ) {
        ...
        switch( icond ) {
          ...
          case 10:
            airlines.add_elem( "ûí" );
            airlines.add_elem( "èé" );
            airlines.add_elem( "çç" );
            airlines.set_elems_permit(true);
            break;
          case 11:
            airps.add_elem( "Ççä" );
...
int test_access(int argc,char **argv)
{
  ...
  //¨†™·®¨†´Ï≠Æ• ™Æ´-¢Æ ¨•·‚
  int l=0;
  for(list<int>::reverse_iterator j=a.rbegin(); j!=a.rend(); ++j)
    if (l+*j<=weight)
    {
      l+=*j;
      printf("%d\n", *j);
  ...
  c=a;
    ...
    k=0;
    for(list<int>::iterator j=b.begin(); j!=b.end();)
    {
      if (k+*j>weight)
      {
        if (true)
    ...
    l=0;
    for(list<int>::reverse_iterator j=c.rbegin(); j!=c.rend();)
    {
      if (l+*j>weight)
      {
        if (true)
        {
          //list<int>::iterator tmp=j.base();
          //++j;
        /*  list<int>::reverse_iterator tmp=j;
          printf("*j=%d *tmp=%d\n", *j, *((++tmp).base()));
          ++j;*/
          c.erase(--j.base());
          continue;
  ...
  s.str("");
  for(list<int>::iterator j=b.begin(); j!=b.end(); ++j) s << *j << " ";
  printf("b: %s\n", s.str().c_str());

  s.str("");
  for(list<int>::iterator j=c.begin(); j!=c.end(); ++j) s << *j << " ";
  printf("c: %s\n", s.str().c_str());
    ...
    for(int j=0x0000; j<0x0008; j++)
    {
      for(int k1=0; k1<3; k1++)
      {
        for(int k2=0; k2<3; k2++)
        {
          ...
          airlines1.build_test(i, "ûí", "èé", "èé");
          airps.build_test(j, "ÑåÑ", "Ççä", "Ççä");
  ...
  for(int k=0; k<3; k++)
  {
    for(int i=0x0000; i<0x0008; i++)
    {
      for(int j=0x0000; j<0x0008; j++)
      {
        ...
        {
          airlines1.build_test(j, "ûí", "èé", "èé");
          airps.build_test(i, "ÑåÑ", "Ççä", "Ççä");
        }
        reqInfo.user.access.merge_airlines(airlines1);
        reqInfo.user.access.merge_airps(airps);
