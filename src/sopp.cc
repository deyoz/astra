...
{
    stringstream ss;
    ss << "SELECT arx_points.move_id,arx_points.point_id,point_num,airp,airp_fmt,first_point,airline,airline_fmt,flt_no, \n"
    "       suffix,suffix_fmt,craft,craft_fmt,bort, \n"
    "       scd_in,est_in,act_in,scd_out,est_out,act_out,trip_type,litera,park_in,park_out,remark, \n"
    ...
    "   WHERE part_key>=:first_date AND part_key < :arx_trip_date_range AND \n"
    "         pr_del <> -1 \n"
    "         :where_sql AND \n"
    "         ( :first_date IS NULL OR \n"
    "           COALESCE(act_in,est_in,scd_in) >= :first_date AND COALESCE(act_in,est_in,scd_in) < :next_date OR \n"
    "           COALESCE(act_out,est_out,scd_out) >= :first_date AND COALESCE(act_out,est_out,scd_out) < :next_date ) \n"
    "  UNION \n"
...
std::string arx_points_ISG_SQL()
{
    ...
    "         ( :first_date IS NULL OR \n"
    "           COALESCE(act_in,est_in,scd_in)    >= :first_date AND COALESCE(act_in,est_in,scd_in)    < :next_date OR \n"
    "           COALESCE(act_out,est_out,scd_out) >= :first_date AND COALESCE(act_out,est_out,scd_out) < :next_date OR \n"
    "           COALESCE(act_in,est_in,scd_in)    IS NULL        AND COALESCE(act_out,est_out,scd_out) IS NULL ) \n"
    "  UNION \n"
    "  SELECT DISTINCT arx_points.move_id, arx_points.part_key \n"
    "  FROM arx_points, \n"
    "       (SELECT part_key, move_id FROM move_arx_ext \n"
...
string getCrsDisplace( int point_id, TDateTime local_time, bool to_local, DB::TQuery &Qry );
...
void read_TripStages( vector<TSoppStage> &stages, TDateTime part_key, int point_id )
{
    if(part_key != NoExists) {
        return arx_read_TripStages(stages, part_key, point_id);
    }
    TCkinClients ckin_clients;
    stages.clear();
...
string addCondition( const char *sql, bool pr_arx )
{
    ...
    bool pr_OR = ( !reqInfo->user.access.airlines().elems().empty() && !reqInfo->user.access.airps().elems().empty() );
  ...
  if ( !reqInfo->user.access.airlines().elems().empty() ) {
   ...
   if ( reqInfo->user.access.airlines().elems_permit() ) {
     ...
     where_sql += GetSQLEnum( reqInfo->user.access.airlines().elems() );
   }
   else {
     if ( pr_arx )
  ...
  if ( !reqInfo->user.access.airps().elems().empty() ) {
    ...
    if ( reqInfo->user.access.airps().elems_permit() ) {
      ...
      where_sql += GetSQLEnum( reqInfo->user.access.airps().elems() );
    }
    else {
        if ( pr_arx )
...
void addCondition_N( DB::TQuery &PointsQry,
                     ...
                     TDateTime next_date)
{
  ...
  if ( !reqInfo->user.access.airps().elems().empty() ) {
    ...
    where_airp_sql += " IN ";
    where_airp_sql += GetSQLEnum( reqInfo->user.access.airps().elems() );
  }
  if ( !reqInfo->user.access.airlines().elems().empty() ) {
    ...
    where_airline_sql += GetSQLEnum( reqInfo->user.access.airlines().elems() );
  }
...
void tstations::fromDB( int point_id, const std::string& work_mode )
{
  ...
  for (; !Qry.Eof; Qry.Next() ) {
    emplace_back( TSOPPStation( Qry.FieldAsString("name"),
                                Qry.FieldAsString("work_mode") ) );
  }
...
void tstations::toDB( const std::string& whereabouts, int point_id, toDbMode mode, const BitSet<toDBModeRewriteAll> &flags  )
{
  ...
  if ( mode == toDbMode::dbWriteReceiveChanged ) { //приходят только изменения
    tstations currStations = dbStations;
  ...
  PrmEnum del_gates_prmenum("names", ",");
  {
    ...
    if ( mode != toDbMode::dbRewriteAll ) {
      if ( !del_terms_prmenum.prms.empty() ) {
        TReqInfo::Instance()->LocaleToLog("EVT.DELETE_DESK", LEvntPrms() << del_terms_prmenum, evtFlt, point_id);
      }
      if ( !del_gates_prmenum.prms.empty() ) {
        TReqInfo::Instance()->LocaleToLog("EVT.DELETE_GATE_ON_FLIGHT", LEvntPrms() << del_gates_prmenum, evtFlt, point_id);
      }
    }
  }
    ...
    for ( const auto &st : chStations ) {
      if ( st.isTerm() ) {
        if ( st.pr_main ) {
          ...
          terms_prmenum.prms << PrmSmpl<std::string>("", st.name);
        }
      }
      else {
        gates_prmenum.prms << PrmSmpl<std::string>("", st.name);
      }
    }
    if ( !terms_prmenum.prms.empty() ) {
      if ( mode == toDbMode::dbRewriteAll ) {
         TReqInfo::Instance()->LocaleToLog("EVT.ASSIGNE_DESKS", LEvntPrms() << terms_prmenum, evtFlt, point_id);
      }
      else {
        TReqInfo::Instance()->LocaleToLog("EVT.ADD_DESKS", LEvntPrms() << terms_prmenum, evtFlt, point_id); //изменения логируем
      }
    }
    else {
      if ( mode == toDbMode::dbRewriteAll && flags.isFlag(toDBModeRewriteAll::dbRewriteAll_Delete_Terms) ) {
        TReqInfo::Instance()->LocaleToLog("EVT.DESKS_NOT_ASSIGNED", evtFlt, point_id);
      }
    }
    if ( !gates_prmenum.prms.empty() ) {
      if ( mode == toDbMode::dbRewriteAll ) {
        TReqInfo::Instance()->LocaleToLog("EVT.ASSIGNE_BOARDING_GATES", LEvntPrms() << gates_prmenum, evtFlt, point_id);
      }
      else {
        TReqInfo::Instance()->LocaleToLog("EVT.ADD_BOARDING_GATES", LEvntPrms() << gates_prmenum, evtFlt, point_id);
      }
    }
    else {
      if ( mode == toDbMode::dbRewriteAll && flags.isFlag(toDBModeRewriteAll::dbRewriteAll_Delete_Gates) ) {
...
string arx_internal_ReadData_N(TSOPPTrips &trips, long int &exec_time, int point_id)
{
  ...
  if ( trips.module == TSOPPTrips::tISG ){
      PointsQry.SQLText = addCondition( arx_points_ISG_SQL().c_str(), trips.pr_arx ).c_str();
  }
  else{
      PointsQry.SQLText = addCondition( arx_points_SOPP_SQL().c_str(), trips.pr_arx ).c_str();
  }
  if ( !pr_addCondition_N ) {
    if ( point_id == NoExists ) {
      if ( trips.first_date != NoExists ) {
        if ( 	TReqInfo::Instance()->user.sets.time == ustTimeLocalAirp ) { // локальные времена пульта в first_date, next_date
            ProgTrace( TRACE5, "ustTimeLocalAirp!!!, first_date=%s, next_date=%s", DateTimeToStr( trips.first_date-1 ).c_str(), DateTimeToStr( trips.next_date+1 ).c_str() );
  // вычитаем сутки, т.к. филтрация идет по UTC, а в случае режима локальных времен может быть переход на
                            // сутки и клиент этот рейс отфильтрует
                            // 20.04.2010 04:00 MEX -> 19.04.2010 22:00 LocalTime
          PointsQry.CreateVariable( "first_date", otDate, trips.first_date-1 ); // UTC +- сутки
          PointsQry.CreateVariable( "next_date", otDate, trips.next_date+1 );
          PointsQry.CreateVariable( "arx_trip_date_range", otDate, trips.next_date+1+ARX_TRIP_DATE_RANGE() );
        }
        else {
          TDateTime f, l;
          modf( trips.first_date, &f );
          modf( trips.next_date, &l );
          PointsQry.CreateVariable( "first_date", otDate, f );
          PointsQry.CreateVariable( "next_date", otDate, l+1 );
          PointsQry.CreateVariable( "arx_trip_date_range", otDate, l+1+ARX_TRIP_DATE_RANGE() );
        }
      }
      else {
      ...
      if ( move_id > NoExists && dests.size() > 1 ) {
        ...
        for( TSOPPDests::iterator id=dests.begin(); id!=dests.end(); id++ ) {
            ...
            if ( reqInfo->user.access.airlines().permitted( airline ) &&
                 reqInfo->user.access.airps().permitted( id->airp ) ) {
                TSOPPTrip ntr = createTrip( move_id, id, dests );
            ntr.ref = ref;
            if ( FilterFlightDate( ntr, trips.first_date, trips.next_date,
                                   errcity, trips.module == TSOPPTrips::tISG ) ) {
                vector<TSOPPTrip>::iterator v=vtrips.end();
                if ( trips.module == TSOPPTrips::tISG && reqInfo->desk.city != "ЧЛБ" ) {
      ...
      for( TSOPPDests::iterator id=dests.begin(); id!=dests.end(); id++ ) {
        ...
        if ( reqInfo->user.access.airlines().permitted( airline ) &&
          ...
          if ( trips.module == TSOPPTrips::tsLibra ) {
            ...
            TTripInfo info;
            info.airline = ntr.airline_out;
            info.flt_no = ntr.flt_no_out;
            info.airp = ntr.airp;
  ...
  if ( trips.module != TSOPPTrips::tsLibra ) {
    for ( TSOPPTrips::iterator tr=trips.begin(); tr!=trips.end(); tr++ ) {
      if ( !tr->places_out.empty() ) {
        // добор информации
        if ( trips.module != TSOPPTrips::tISG ) {
          cfg.get( tr->point_id, tr->part_key );
          tr->cfg.insert( tr->cfg.begin(), cfg.begin(), cfg.end() );
        } // module != tISG
        if ( trips.module == TSOPPTrips::tSPPCEK )
         continue;
...
std::string makeTripString(const std::string& airline,
                           const std::string& flt_no,
                           const std::string& suffix,
                           TDateTime scd,
                           TDateTime local_time)
{
  std::string trip = ElemIdToCodeNative( etAirline, airline )
      + flt_no
...
string makeXmlCrsDisplaceTo(const PointId_t& point_id, TDateTime local_time)
{
  string result;
  ...
  for (const PointIdTlg_t& point_id_tlg: point_id_tlg_set) {
    ...
    Qry.SQLText =
        "SELECT point_id_spp FROM crs_displace2 "
    ...
    for(;!Qry.Eof;Qry.Next()) {
      ...
      if ( point_id_spp != point_id ) {
        TDateTime scd = fltInfo.scd_out;
        string region = AirpTZRegion( fltInfo.airp, true );
        scd = UTCToLocal( scd, region );
        const std::string trip = makeTripString(fltInfo.airline,
                                                std::to_string(fltInfo.flt_no),
                                                fltInfo.suffix, scd, local_time);
...
string makeXmlCrsDisplaceFrom(const PointId_t& point_id, TDateTime local_time)
{
  DB::TQuery Qry(PgOra::getROSession("CRS_DISPLACE2"), STDLOG);
  Qry.SQLText =
      "SELECT class_spp,airp_arv_spp,airline,flt_no,suffix,scd, "
      "       class_tlg,airp_arv_tlg,point_id_tlg "
      " FROM crs_displace2 "
      " WHERE point_id_spp=:point_id_spp "
      " ORDER BY airline,flt_no,suffix,scd ";
  ...
  if ( ch_dest ) {
    result = AstraLocale::getLocaleText("Изм. пункта") + " " + result;
  }
  return result;
}

string internal_ReadData_N( TSOPPTrips &trips, long int &exec_time, int point_id )
{
    if (trips.pr_arx) {
        return arx_internal_ReadData_N(trips, exec_time, point_id);
    }
    string errcity;
    TReqInfo *reqInfo = TReqInfo::Instance();
  ...
  if ( trips.module != TSOPPTrips::tsLibra ) {
    for ( TSOPPTrips::iterator tr=trips.begin(); tr!=trips.end(); tr++ ) {
      if ( !tr->places_out.empty() ) {
        // добор информации
        if ( trips.module != TSOPPTrips::tISG ) {
          cfg.get( tr->point_id, ASTRA::NoExists );
          tr->cfg.insert( tr->cfg.begin(), cfg.begin(), cfg.end() );
        } // module != tISG
        if ( trips.module == TSOPPTrips::tSPPCEK )
         continue;
        ...
        try {
          ...
          tr->crs_disp_to = makeXmlCrsDisplaceTo(PointId_t(tr->point_id), local_time);
          tr->crs_disp_from = makeXmlCrsDisplaceFrom(PointId_t(tr->point_id), local_time);
        }
        catch( Exception &e ) {
           if ( errcity.empty() )
             errcity = tr->city;
...
string arx_internal_ReadData( TSOPPTrips &trips, TDateTime first_date, TDateTime next_date,
                          TSOPPTrips::TModule module, long int &exec_time, int point_id = NoExists )
{
    string errcity;
    TReqInfo *reqInfo = TReqInfo::Instance();
  ...
  TBaseTable &cities = base_tables.get( "cities" );
    ...
    if ( module == TSOPPTrips::tISG ) {
        PointsQry.SQLText = addCondition(arx_points_ISG_SQL().c_str(), true).c_str();
    } else {
        PointsQry.SQLText = addCondition(arx_points_SOPP_SQL().c_str(), true).c_str();
    }
...
// вычитаем сутки, т.к. филтрация идет по UTC, а в случае режима локальных времен может быть переход на
        ...
        PointsQry.CreateVariable( "arx_trip_date_range", otInteger, next_date+1+ARX_TRIP_DATE_RANGE() );
      }
      else {
        TDateTime f, l;
        modf( first_date, &f );
        ...
        PointsQry.CreateVariable( "first_date", otDate, f );
        PointsQry.CreateVariable( "next_date", otDate, l+1 );
        PointsQry.CreateVariable( "arx_trip_date_range", otInteger, l+1+ARX_TRIP_DATE_RANGE() );
      }
      ...
      if ( move_id > NoExists && dests.size() > 1 ) {
        ...
        for( TSOPPDests::iterator id=dests.begin(); id!=dests.end(); id++ ) {
            ...
            if ( FilterFlightDate( ntr, first_date, next_date,
                                     errcity, module == TSOPPTrips::tISG ) ) {
                vector<TSOPPTrip>::iterator v=vtrips.end();
                if ( module == TSOPPTrips::tISG && reqInfo->desk.city != "ЧЛБ" ) {
                  for (v=vtrips.begin(); v!=vtrips.end(); v++) {
                    if ( EqualTrips( ntr, *v ) )
                        break;
                }
  ...
  if ( move_id > NoExists ) {
    ...
    for(TSOPPDests::iterator id=dests.begin(); id!=dests.end(); id++ ) {
           ...
           reqInfo->user.access.airps().permitted(id->airp))
      {
         TSOPPTrip ntr = createTrip(move_id, id, dests);
         ntr.ref = ref;
         if (FilterFlightDate(ntr, first_date, next_date, errcity, module == TSOPPTrips::tISG)) {
            vector<TSOPPTrip>::iterator v=vtrips.end();
            if (module == TSOPPTrips::tISG && reqInfo->desk.city != "ЧЛБ") {
              for (v=vtrips.begin(); v!=vtrips.end(); v++) {
                if (EqualTrips(ntr, *v))
...
string internal_ReadData( TSOPPTrips &trips, TDateTime first_date, TDateTime next_date,
                          bool arx, TSOPPTrips::TModule module, long int &exec_time, int point_id = NoExists )
{
    if(arx) {
        return arx_internal_ReadData(trips, first_date, next_date, module, exec_time, point_id);
    }
    string errcity;
    TReqInfo *reqInfo = TReqInfo::Instance();
...
//  createCentringFile( 13672, "ASTRA", "DMDTST" );
  ...
  TSOPPTrips::TModule module;

  if ( GetNode( "disp_isg", reqNode ) )
    module = TSOPPTrips::tISG;
  else
    module = TSOPPTrips::tSOPP;
  ...
  TSOPPTrips trips( first_date, next_date, arx, module );
  string errcity = internal_ReadData_N( trips, exec_time );
  if ( module == TSOPPTrips::tISG )
                       ...
                       map<int,TAdvTripInfo> &segs )
{
  ...
  if (!fltInfo.getByPointId(point_id, FlightProps(FlightProps::NotCancelled,
  ...
  TCkinQry.SQLText=
    ...
    "      tckin_id=:tckin_id AND grp_num>:grp_num "
    "ORDER BY grp_num";
  ...
  for(;!Qry.Eof;Qry.Next())
  {
    ...
    int tckin_grp_num=Qry.FieldIsNULL("grp_num")?NoExists:Qry.FieldAsInteger("grp_num");

    if ( filter.inbound_point_dep!=NoExists )
    {
      if (tckin_id==NoExists || tckin_grp_num==NoExists) continue;
      auto inboundGrp=TCkinRoute::getPriorGrp(tckin_id,
                                              tckin_grp_num,
    ...
    if (tckin_id!=NoExists && tckin_grp_num!=NoExists)
    {
      if (SeparateTCkin(grp_id,cssAllPrevCurr,cssNone,NoExists)!=NoExists)
      {
        ...
        TCkinQry.SetVariable("grp_num",tckin_grp_num);
        TCkinQry.Execute();
...
void DeletePassengersAnswer( map<int,TAdvTripInfo> &segs, xmlNodePtr resNode )
{
  ...
  Qry.DeclareVariable("point_id",otInteger);
    ...
    for(map<int,TAdvTripInfo>::const_iterator i=segs.begin();i!=segs.end();++i)
  {
    bool pr_etl_only=GetTripSets(tsETSNoExchange,i->second);
...
void SoppInterface::DeleteAllPassangers(XMLRequestCtxt *ctxt, xmlNodePtr reqNode, xmlNodePtr resNode)
{
  ...
  if (filter.inbound_point_dep==NoExists)
  {
    ...
    TSOPPTrips trips( ASTRA::NoExists, ASTRA::NoExists, false, TSOPPTrips::tSOPP );
    long int exec_time;
    string errcity = internal_ReadData( trips, NoExists, NoExists, false, TSOPPTrips::tSOPP, exec_time, point_id );
    xmlNodePtr dataNode = NewTextChild( resNode, "data" );
    buildSOPP( trips, errcity, dataNode );
    if ( !errcity.empty() )
    {
    ...
    while ( node ) {
      ...
      if ( ddddNode ) {
        ...
        while ( ddddNode ) {
          ...
          if ( TSOPPStation::isTerm(work_mode) ) {
            flags.setFlag( tstations::toDBModeRewriteAll::dbRewriteAll_Delete_Terms );
          }
          if ( TSOPPStation::isGate(work_mode) ) {
            flags.setFlag( tstations::toDBModeRewriteAll::dbRewriteAll_Delete_Gates );
          }
          ...
          while ( stnode ) {
            ...
            if ( terms.find( name ) == terms.end() ) {
              terms.insert( name );
              stations.emplace_back( TSOPPStation(name,work_mode,GetNode( "pr_main", stnode )) );
            }
            stnode = stnode->next;
          }
          ddddNode = ddddNode->next;
      }
      stations.toDB( "sopp", point_id, tstations::toDbMode::dbRewriteAll, flags );
    }
        ...
        if ( luggageNode && r->user.access.rights().permitted(370) ) {
...
void readDestsFromDb(TSOPPDests &dests, DB::TQuery& Qry, DB::TQuery& DQry)
{
    while ( !Qry.Eof ) {
        TSOPPDest d;
                          ...
                          xmlNodePtr resNode, TSoppWriteOwner owner )
{
  ...
  for( TSOPPDests::iterator id=dests.begin(); id!=dests.end(); id++ ) {
    ...
    if ( insert_point ) {
     ...
     DB::TQuery getQry(PgOra::getROSession("POINTS"), STDLOG);
     getQry.SQLText =
      ...
      "FROM points "
      "WHERE point_id=:point_id ";
      getQry.CreateVariable( "point_id", otInteger, id->point_id );
      getQry.Execute();
      if ( getQry.FieldIsNULL( "point_num" ) )
        old_dest.point_num = NoExists;
      else
        old_dest.point_num = getQry.FieldAsInteger( "point_num" );
      old_dest.airp = getQry.FieldAsString( "airp" );
      old_dest.airp_fmt = (TElemFmt)getQry.FieldAsInteger( "airp_fmt" );
      old_dest.pr_tranzit = getQry.FieldAsInteger( "pr_tranzit" );
      if ( getQry.FieldIsNULL( "first_point" ) )
        old_dest.first_point = NoExists;
      else
        old_dest.first_point = getQry.FieldAsInteger( "first_point" );
      if ( getQry.FieldIsNULL( "airline" ) )
        old_dest.airline.clear();
      else {
        old_dest.airline = getQry.FieldAsString( "airline" );
        old_dest.airline_fmt = (TElemFmt)getQry.FieldAsInteger( "airline_fmt" );
      }
      if ( getQry.FieldIsNULL( "flt_no" ) )
        old_dest.flt_no = NoExists;
      else
        old_dest.flt_no = getQry.FieldAsInteger( "flt_no" );
      if ( getQry.FieldIsNULL( "suffix" ) )
        old_dest.suffix.clear();
      else {
        old_dest.suffix = getQry.FieldAsString( "suffix" );
        old_dest.suffix_fmt = (TElemFmt)getQry.FieldAsInteger( "suffix_fmt" );
      }
      old_dest.craft = getQry.FieldAsString( "craft" );
      old_dest.craft_fmt = (TElemFmt)getQry.FieldAsInteger( "craft_fmt" );
      old_dest.bort = getQry.FieldAsString( "bort" );
      if ( getQry.FieldIsNULL( "scd_in" ) )
        old_dest.scd_in = NoExists;
      else
        old_dest.scd_in = getQry.FieldAsDateTime( "scd_in" );
      if ( getQry.FieldIsNULL( "est_in" ) )
        old_dest.est_in = NoExists;
      else
        old_dest.est_in = getQry.FieldAsDateTime( "est_in" );
      if ( getQry.FieldIsNULL( "act_in" ) )
        old_dest.act_in = NoExists;
      else
        old_dest.act_in = getQry.FieldAsDateTime( "act_in" );
      if ( getQry.FieldIsNULL( "scd_out" ) )
        old_dest.scd_out = NoExists;
      else
        old_dest.scd_out = getQry.FieldAsDateTime( "scd_out" );
      if ( getQry.FieldIsNULL( "est_out" ) )
        old_dest.est_out = NoExists;
      else
        old_dest.est_out = getQry.FieldAsDateTime( "est_out" );
      if ( getQry.FieldIsNULL( "act_out" ) )
        old_dest.act_out = NoExists;
      else
        old_dest.act_out = getQry.FieldAsDateTime( "act_out" );
      old_dest.triptype = getQry.FieldAsString( "trip_type" );
      old_dest.litera = getQry.FieldAsString( "litera" );
      old_dest.pr_del = getQry.FieldAsInteger( "pr_del" );
      old_dest.pr_reg = getQry.FieldAsInteger( "pr_reg" );
      old_dest.remark = getQry.FieldAsString( "remark" );
  ...
  if ( ch_point_num ) {
    DB::TQuery Qry(PgOra::getRWSession("POINTS"), STDLOG);
    Qry.SQLText = "UPDATE points SET point_num=:point_num "
                  "WHERE point_id=:point_id";
    Qry.DeclareVariable( "point_id", otInteger );
...
void SoppInterface::ReadCRS_Displaces(XMLRequestCtxt *ctxt, xmlNodePtr reqNode, xmlNodePtr resNode) /*???*/
{
    ...
    Qry1.SQLText = "SELECT airp,first_point,pr_tranzit,airp_fmt "
                  "FROM points WHERE point_id=:point_id AND pr_del <> -1";
    Qry1.CreateVariable( "point_id", otInteger, point_id );
    Qry1.Execute();
    if ( Qry1.Eof )
        throw AstraLocale::UserException( "MSG.FLIGHT.NOT_FOUND" );
    int pr_tranzit = ( !Qry1.FieldIsNULL( "first_point" ) && Qry1.FieldAsInteger( "pr_tranzit" ) );
    string airp_dep = Qry1.FieldAsString( "airp" );
   ...
   "ORDER BY p2.point_num ASC";
    ...
    while ( !Qry2.Eof ) {
        ...
        NewTextChild( displnode, "airp_arv", ElemIdToCodeNative( etAirp, Qry2.FieldAsString( "airp" ) ) );
        Qry2.Next();
    }
    NewTextChild( crsdNode, "airp_dep", ElemIdToCodeNative( etAirp, airp_dep ) );
    if ( pr_tranzit )
      NewTextChild( crsdNode, "pr_tranzit", pr_tranzit );
    DB::TQuery Qry3(PgOra::getROSession("CRS_DISPLACE2"), STDLOG);
    Qry3.SQLText =
  ...
  while ( !Qry3.Eof ) {
    ...
    xmlNodePtr snode = NewTextChild( displnode, "displace" );
        NewTextChild( snode, "airp_arv_spp", ElemIdToCodeNative( etAirp, Qry3.FieldAsString( "airp_arv_spp" ) ) );
        NewTextChild( snode, "class_spp", ElemIdToCodeNative( etClass, Qry3.FieldAsString( "class_spp" ) ) );
        NewTextChild( snode, "airline", ElemIdToCodeNative( etAirline, Qry3.FieldAsString( "airline" ) ) );
        NewTextChild( snode, "flt_no", Qry3.FieldAsString( "flt_no" ) );
        NewTextChild( snode, "suffix", ElemIdToCodeNative( etSuffix, Qry3.FieldAsString( "suffix" ) ) );
        NewTextChild( snode, "scd", DateTimeToStr( Qry3.FieldAsDateTime( "scd" ) ) );
        NewTextChild( snode, "airp_dep", ElemIdToCodeNative( etAirp, Qry3.FieldAsString( "airp_dep" ) ) );
        NewTextChild( snode, "airp_arv_tlg", ElemIdToCodeNative( etAirp, Qry3.FieldAsString( "airp_arv_tlg" ) ) );
        NewTextChild( snode, "class_tlg", ElemIdToCodeNative( etClass, Qry3.FieldAsString( "class_tlg" ) ) );
      NewTextChild( snode, "status", Qry3.FieldAsString( "status" ) );
    Qry3.Next();
  }
  /* pr_utc */
    ...
    Qry4.SQLText =
     ...
     "      scd >= :yesterday AND scd < :after_tomorrow "
     "ORDER BY scd,airline,flt_no,suffix,airp_arv ";
    Qry4.CreateVariable( "airp_dep", otString, airp_dep );
    double today;
    modf(BASIC::date_time::NowUTC(), &today);
    const BASIC::date_time::TDateTime yesterday = today - 1;
    const BASIC::date_time::TDateTime after_tomorrow = today + 2;
    Qry4.CreateVariable( "yesterday", otDate, yesterday );
    Qry4.CreateVariable( "after_tomorrow", otDate, after_tomorrow );
    Qry4.Execute();
    displnode = NULL;
    while ( !Qry4.Eof ) {
        if ( !displnode )
        ...
        NewTextChild( snode, "airline", ElemIdToCodeNative(etAirline, Qry4.FieldAsString( "airline" ) ) );
        NewTextChild( snode, "flt_no", Qry4.FieldAsString( "flt_no" ) );
        NewTextChild( snode, "suffix", ElemIdToCodeNative(etSuffix, Qry4.FieldAsString( "suffix" ) ) );
        NewTextChild( snode, "scd", DateTimeToStr( Qry4.FieldAsDateTime( "scd" ) ) );	  	//???
      Qry4.Next();
    }
}

std::string getPoint_airp_dep(const PointId_t& point_id)
{
  DB::TQuery Qry(PgOra::getROSession("POINTS"), STDLOG);
  Qry.SQLText = "SELECT airp FROM points "
                "WHERE point_id=:point_id AND pr_del <> -1";
  ...
  if ( Qry.Eof )
    throw AstraLocale::UserException( "MSG.FLIGHT.NOT_FOUND" );
  return Qry.FieldAsString( "airp" );
}

std::vector<tcrs_displ> loadCrsDisplaces(const PointId_t& point_id)
{
  std::vector<tcrs_displ> result;
  DB::TQuery Qry(PgOra::getROSession("CRS_DISPLACE2"), STDLOG);
  Qry.SQLText =
...
void SoppInterface::WriteCRS_Displaces(XMLRequestCtxt *ctxt, xmlNodePtr reqNode, xmlNodePtr resNode)
{
  ...
  TQry.DeclareVariable( "scd_tomorrow", otDate );
  const string airp_dep = getPoint_airp_dep(point_id);
  const string region = AirpTZRegion( airp_dep, true );
  TDateTime local_time;
  modf(UTCToLocal( NowUTC(), region ),&local_time);
  std::vector<tcrs_displ> crs_displaces = loadCrsDisplaces(point_id);
  deleteCrsDisplaces(point_id);

  DB::TQuery QryInsert(PgOra::getRWSession("CRS_DISPLACE2"), STDLOG);
  QryInsert.SQLText =
      ...
      "VALUES (:point_id_spp,:airp_arv_spp,:class_spp,:airline,:flt_no,:suffix,:scd,:airp_dep, "
      "        :point_id_tlg,:airp_arv_tlg,:class_tlg,:status) ";
  QryInsert.CreateVariable( "point_id_spp", otInteger, point_id.get() );
  QryInsert.DeclareVariable( "airp_arv_spp", otString );
  QryInsert.DeclareVariable( "class_spp", otString );
  ...
  QryInsert.DeclareVariable( "airp_arv_tlg", otString );
  QryInsert.DeclareVariable( "class_tlg", otString );
  QryInsert.DeclareVariable( "status", otString );
  QryInsert.DeclareVariable( "point_id_tlg", otInteger );
  ...
  while ( node ) {
    ...
    QryInsert.SetVariable( "airp_arv_spp", airp_arv_spp );
    QryInsert.SetVariable( "class_spp", class_spp );
    QryInsert.SetVariable( "airline", airline );
    QryInsert.SetVariable( "flt_no", flt_no );
    QryInsert.SetVariable( "suffix", suffix );
    QryInsert.SetVariable( "scd", scd );
    QryInsert.SetVariable( "airp_dep", to_airp_dep );
    QryInsert.SetVariable( "airp_arv_tlg", airp_arv_tlg );
    QryInsert.SetVariable( "class_tlg", class_tlg );
    QryInsert.SetVariable( "status", status );
    ...
    TQry.SetVariable( "scd", first_day );
    TQry.SetVariable( "scd_tomorrow", last_day );
    TQry.Execute();
    if ( !TQry.Eof )
    ...
    if ( point_id_tlg == NoExists )
        QryInsert.SetVariable( "point_id_tlg", FNull );
    else
        QryInsert.SetVariable( "point_id_tlg", point_id_tlg );
    QryInsert.Execute();
    auto r = crs_displaces.end();
    for ( r=crs_displaces.begin(); r!=crs_displaces.end(); r++ ) {
        if ( r->point_id_spp == point_id.get() &&
    ...
    if ( r != crs_displaces.end() ) {
        ...
        params << sts;
        TReqInfo::Instance()->LocaleToLog("EVT.RESEAT_MODIFY", params, evtFlt, point_id.get());
      }
      ...
      TReqInfo::Instance()->LocaleToLog("EVT.RESEAT_ADD", params, evtFlt, point_id.get());
    node = node->next;
    }
  }
  ...
  NewTextChild( tripNode, "to", makeXmlCrsDisplaceTo(point_id, local_time));
  NewTextChild( tripNode, "from", makeXmlCrsDisplaceFrom(point_id, local_time));
  AstraLocale::showMessage( "MSG.DATA_SAVED" );
}
...
{
  long int exec_time;
  internal_ReadData( trips, NoExists, NoExists, false, TSOPPTrips::tSPPCEK, exec_time, point_id );
}
...
void ChangeTrip( int point_id, TSOPPTrip tr1, TSOPPTrip tr2, BitSet<TSOPPTripChange> FltChange )
{
  ...
  if ( (FltChange.isFlag( tsNew ) ||
        ...
        FltChange.isFlag( tsAttr ) ) ) { // восстановление (новый)
    if ( tr1.pr_del_out != -1 &&
...
void SoppInterface::GetTime(XMLRequestCtxt *ctxt, xmlNodePtr reqNode, xmlNodePtr resNode)
{
    ...
    TDateTime time = UTCToClient( NowUTC(), region );
                     ...
                     bool &trfer_exists, string &ckin_desks, string &gates )
{
  ...
  DB::TQuery QryTrip(PgOra::getROSession("TRIP_CALC_DATA"), STDLOG);
  QryTrip.SQLText =
    "SELECT trfer_exists, ckin_desks, gates "
    "FROM trip_calc_data "
    "WHERE point_id=:point_id";
  QryTrip.CreateVariable( "point_id", otInteger, point_id );
  QryTrip.Execute();
  bool pr_empty = QryTrip.Eof;
  if ( !pr_empty ) {
    trfer_exists = QryTrip.FieldAsInteger( "trfer_exists" );
    ckin_desks = QryTrip.FieldAsString( "ckin_desks" );
    gates = QryTrip.FieldAsString( "gates" );
  }
...
void getVouchers( std::set<std::string> &vouchers)
{
  vouchers.clear();
  DB::TQuery Qry(PgOra::getROSession("VOUCHER_TYPES"), STDLOG);
  Qry.SQLText =
...
{
  trip_vouchers.clear();
  DB::TQuery Qry(PgOra::getROSession("TRIP_VOUCHERS"), STDLOG);
  Qry.SQLText =
    "SELECT voucher_code FROM trip_vouchers "
    "WHERE point_id=:point_id";
  Qry.CreateVariable( "point_id", otInteger, point_id );
  Qry.Execute();
  for ( ; !Qry.Eof; Qry.Next() ) {
...
void SoppInterface::WriteVoucher(XMLRequestCtxt *ctxt, xmlNodePtr reqNode, xmlNodePtr resNode)
{
  ...
  getTripVouchers(point_id, vouchers_old);
  ...
  DB::TQuery delQry(PgOra::getRWSession("TRIP_VOUCHERS"), STDLOG);
  delQry.SQLText =
    "DELETE trip_vouchers "
    "WHERE point_id=:point_id "
    "AND voucher_code=:voucher_code";
  delQry.CreateVariable( "point_id", otInteger, point_id );
  delQry.DeclareVariable( "voucher_code", otString );
  ...
  for ( std::set<std::string>::iterator ov=vouchers_old.begin(); ov!=vouchers_old.end(); ov++ ) {
    ...
    delQry.SetVariable( "voucher_code", *ov );
    delQry.Execute();
    del_prmenum.prms << PrmSmpl<std::string>("", ElemIdToNameLong( etVoucherType, *ov ) );
  }
  DB::TQuery insQry(PgOra::getRWSession("TRIP_VOUCHERS"), STDLOG);
  insQry.SQLText =
    "INSERT INTO trip_vouchers(point_id,voucher_code) "
    "VALUES(:point_id,:voucher_code)";
  insQry.CreateVariable( "point_id", otInteger, point_id );
  insQry.DeclareVariable( "voucher_code", otString );
  for ( std::set<std::string>::iterator nv=vouchers_new.begin(); nv!=vouchers_new.end(); nv++ ) {
    if ( vouchers_old.find( *nv ) != vouchers_old.end() ) {
      continue;
    }
    insQry.SetVariable( "voucher_code", *nv );
    insQry.Execute();
...
{
  if (flt.first_point==ASTRA::NoExists ||
      flt.pr_tranzit==new_pr_tranzit)
  {
    return;
  }

  QParams params;
  params << QParam("pr_tranzit",otInteger,(int)new_pr_tranzit)
         << QParam("point_id",otInteger,flt.point_id)
         << QParam("first_point",otInteger,flt.first_point)
         << QParam("point_num",otInteger,flt.point_num);

  {
    DB::TCachedQuery Qry(
          PgOra::getRWSession("POINTS"),
          "UPDATE points SET "
          "pr_tranzit=:pr_tranzit "
          "WHERE point_id = :point_id "
          "AND pr_del >= 0 ",
          params,
          STDLOG);
  ...
  TReqInfo::Instance()->LocaleToLog(new_pr_tranzit?
                                      "EVT.TRANSIT_FLIGHT.SET":
                                      "EVT.TRANSIT_FLIGHT.CANCEL",
                                    evtFlt, flt.point_id);
}
...
"ORDER BY class, priority DESC";
  ...
  boost::optional<bool> pr_tranzit;
  newSetList.append(TTripSetList().getTransitSets(flt, pr_tranzit));

  if (flt.first_point!=NoExists && pr_tranzit)
    updateTransitIfNeeded(flt, pr_tranzit.get());  //!!!djek функция должна быть более серьезной - взять куски из prepreg.cc
...
static void setStageSettings(const TTripInfo& fltInfo, StageInitPropsContainer& initProps)
{
  ...
  Qry.SQLText=
    ...
    "       CASE WHEN airp IS NULL THEN 0 ELSE 1 END AS priority "
    "FROM stage_sets "
    "WHERE (airline IS NULL OR airline=:airline) AND "
    "      (airp IS NULL OR airp=:airp) "
    "ORDER BY stage_id, priority DESC";
  Qry.CreateVariable("airline", otString, fltInfo.airline);
  Qry.CreateVariable("airp", otString, fltInfo.airp);
  Qry.Execute();
  boost::optional<TStage> priorStageId(sNoActive);
  priorStageId=boost::none;
  for(;!Qry.Eof;Qry.Next())
  {
    TStage stageId=static_cast<TStage>(Qry.FieldAsInteger("stage_id"));
    if (!priorStageId || priorStageId.get()!=stageId)
    {
      initProps.setAutoAttribute(stageId, Qry.FieldAsInteger("pr_auto")!=0);
...
static void setStageTimes(const TTripInfo& fltInfo, StageInitPropsContainer& initProps)
{
  ...
  Qry.SQLText=
    ...
    "       CASE WHEN airline IS NULL THEN 0 ELSE 8 END + "
    "       CASE WHEN airp IS NULL THEN 0 ELSE 4 END + "
    "       CASE WHEN craft IS NULL THEN 0 ELSE 2 END + "
    "       CASE WHEN trip_type IS NULL THEN 0 ELSE 1 END AS priority "
    "FROM graph_times "
    "WHERE ( airline IS NULL OR airline = :airline ) AND "
    "      ( airp IS NULL OR airp = :airp ) AND "
    "      ( craft IS NULL OR craft = :craft ) AND "
    "      ( trip_type IS NULL OR trip_type = :trip_type ) "
    "ORDER BY stage_id, priority DESC";
  Qry.CreateVariable("airline", otString, fltInfo.airline);
  Qry.CreateVariable("airp", otString, fltInfo.airp);
  ...
  for(;!Qry.Eof;Qry.Next())
  {
    ...
    if (!priorStageId || priorStageId.get()!=stageId)
    {
      initProps.setTimeBeforeScdOut(stageId, Qry.FieldAsInteger("time"));
      priorStageId=stageId;
    }
  }
}
...
static void putTripStages(int point_id)
{
  ...
  StageInitPropsContainer initProps;
  setStageSettings(flt, initProps);
  ...
  InsQry.DeclareVariable("scd", otDate);
  ...
  for(;!GraphQry.Eof;GraphQry.Next())
  {
    ...
    LEvntPrms params;
    params << PrmStage("name", stageId, flt.airp);
    if (scd!=NoExists)
      params << PrmDate("time", scd, "hh:nn dd.mm.yy (UTC)");
    else
