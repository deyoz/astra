                      ...
                      std::ofstream& farv, std::ofstream& fdep)
{
    ...
    auto cur = make_db_curs(
                ...
                "      arx_pax_grp.status NOT IN ('E') AND (arx_pax_grp.airp_dep='БЕД' OR arx_pax_grp.airp_arv='БЕД') AND "
    ...
    while(!cur.fen())
    {
        ...
        for(int pass=0; pass<2; pass++)
        {
            ...
            if ((pass==0 && arx_pax_grp_airp_arv != "БЕД") ||
                    (pass!=0 && arx_pax_grp_airp_dep != "БЕД")) continue;
