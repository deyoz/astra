...
void TSelfCkinSalonTariff::setTariffMap( const std::string &airline,
  ...
  DB::TQuery Qry(PgOra::getROSession("RFISC_RATES_SELF_CKIN"), STDLOG);
  Qry.SQLText =
    "SELECT "
    "  rfisc, "
    "  rate, "
    ...
    "   + CASE WHEN airp_arv = :airp_arv THEN 100 WHEN airp_arv IS NULL THEN 50 ELSE 0 END "
    "   + CASE WHEN craft = :craft THEN 10 WHEN craft IS NULL THEN 5 ELSE 0 END "
    "   ) AS priority "
...
void TPlace::AddLayer( int key, const TLayerPrioritySeat &layerPrioritySeat ) { //сортировка по маршруту point_id, вначале вызов с ближними пунктами, далее более дальними
  if ( isCleanDoubleLayerType( layerPrioritySeat.layerType() ) ) {
    for ( std::map<int, TSetOfLayerPriority,classcomp >::iterator ilayers=lrss.begin();
      ...
      if ( key == ilayers->first ) { //дошли до пункта разметки
      ...
      for ( TSetOfLayerPriority::iterator ilayer= ilayers->second.begin();
        ...
        if ( ilayer->layerType() == layerPrioritySeat.layerType() &&
          ...
          //требуется добавлять поздний слой, т.к. он болеее приоритетен
   ...
   if ( RFISCMode != rRFISC ||
     ...
     if ( RFISCMode != rTariff &&
       ...
       for ( std::map<int, TRFISC,classcomp>::iterator irfisc=rfiscs.begin();
         ...
         seatTariff.rate = 1; //!!!
         seatTariff.currency_id = ElemIdToClientElem( etCurrency, "РУБ", efmtCodeNative ); //!!!
  ...
  for ( ; !Qry.Eof; Qry.Next() ) {
    ...
    if ( !findSeat( salons, &placelist, point_s ) ) {
      if ( filterSets.filterClass.empty() &&
           !SALONS2::isFreeSeating( getDepartureId() ) ) {
  ...
  for ( ; !Qry.Eof; Qry.Next() ) {
    ...
    if ( col_point_id < 0 ||
      ...
      if ( layerPrioritySeat.getPaxId() != NoExists &&
        ...
        auto iranges = pax_list[ layerPrioritySeat.getPaxId() ].total_ranges.find( layerPrioritySeat );
...
//log                 placelist, point_s.num, point_s.x, point_s.y, layerPrioritySeat.toString().c_str() );
      ...
      if ( place->isCleanDoubleLayerType( layerPrioritySeat.layerType() ) ) {
         pr_add = filterRoutes.useRouteProperty( layerPrioritySeat.point_id(), layerPrioritySeat.point_arv() ); //разметка должна быть внутри маршрута
         //а нет ли по маршруту слоя признака перерегистрации транзита?
      }
      if ( pr_add ) {
        place->AddLayer( layerPrioritySeat.point_id(), layerPrioritySeat ); //!!!важна сортировка point_id для addLayer
...
bool TSalonList::isRefused( int point_dep, int pax_id ) const {
  std::map<int,TPaxList>::const_iterator pxs = pax_lists.find( point_dep );
  if ( pxs == pax_lists.end() )
    return false;
  LogTrace(TRACE5) << point_dep << " " << pax_id << " " << ( pxs->second.refuseds.find(pax_id) != pxs->second.refuseds.end() );
  return ( pxs->second.refuseds.find(pax_id) != pxs->second.refuseds.end() );
}
...
void TSalonList::ReadPaxs( DB::TQuery &Qry, TPaxList &pax_list )
{
  ...
  for ( ; !Qry.Eof; Qry.Next() ) {
    ...
    int idx_refuse = Qry.FieldIndex( "refuse" );
    if ( !Qry.FieldIsNULL(idx_refuse) ) {
      LogTrace(TRACE5) << "refused " << Qry.FieldAsInteger( idx_pax_id );
      pax_list.refuseds.emplace( Qry.FieldAsInteger( idx_pax_id ) );
      continue;
    }
    TPass pass;
...
//!log                 iroute->point_id,  Qry.FieldAsInteger( "pr_tranzit" ), !Qry.FieldIsNULL( "act_out" ) );
  ...
  if ( routes.empty() && point_arv == ASTRA::NoExists ) { //нет вылета
      point_arv = point_dep;
  }
  if ( point_arv != ASTRA::NoExists ) {
                                           ...
                                           int point_dep2, int point_arv2, bool pr_routes )
{
  ...
  //не пересекаются, а с заданным маршрутом A-B-C-D. 1: A->C, 2: C->D, маршрут отображения B->D
  if ( pr_routes ) {
    return false;
  }
  bool res1, res2;
...
void TSalonList::ReadCompon( int vcomp_id, int point_id )
{
  ...
  ReadLayers( CompBaseLyrsQry, filterRoutes, filterLayers, pax_list, params );
                       ...
                       const TWaitListReason &waitListReason )
{
  if ( layer.getPaxId() == ASTRA::NoExists ) {
    ...
    if ( isBlockedLayer( layer.layerType() ) ) { //это блокирующий слой, надо удалить все слои под ним
...
//log                 prior_layer.toString().c_str(), curr_layer.toString().c_str() );
      if ( filterRoutes.IntersecRoutes( prior_layer.point_dep()==NoExists?prior_layer.point_id():prior_layer.point_dep(),
                                        ...
                                        curr_layer.point_arv(), false ) ) { //есть пересечение  - сравниваем предыдущий с последующим
  ...
  for ( std::map<int,TPaxList>::iterator iroute_pax_list=pax_lists.begin();
    ...
    for ( TPaxList::iterator ipax_list=iroute_pax_list->second.begin();
      ...
      for ( TLayersPax::iterator ilayer=ipax_list->second.layers.begin();
        ...
        if ( ilayer->second.waitListReason.status != layerValid ) {  //сохраняем только валидные слои
                             ...
                             const TSalonListReadParams &params )
{
  ...
  if ( /*!for_calc_waitlist*/ !params.for_get_seat_no ) { //при расчете WL требуются ремарки
  ...
  if ( /*!for_calc_waitlist*/ !params.for_get_seat_no ) { //в расчете WL уже требуются тарифы
    //начитываем тарифы мест по маршруту
...
//      ProgTrace( TRACE5, "TSalonList::ReadFlight: crs_pax_lists[ %d ].size()=%zu", iseg->point_id, pax_lists[ iseg->point_id ].size() );
  ...
  TTQry.SQLText =
    ...
    "   RIGHT OUTER JOIN trip_comp_layers "
  ...
  CommitLayers();
  if ( params.for_get_seat_no ) {
    DB::TQuery TTlgQry(PgOra::getROSession({"TLG_BINDING","TLG_COMP_LAYERS"}), STDLOG);
    TTlgQry.SQLText =
    ...
    TTlgQry.DeclareVariable( "point_id", otInteger );
    for ( std::vector<TTripRouteItem>::const_iterator iseg=filterRoutes.begin();
          iseg!=filterRoutes.end(); iseg++ ) {
      if ( iseg->point_id != filterRoutesSets.point_dep ) { //только по нашему пп
        continue;
      }
      TTlgQry.SetVariable( "point_id", iseg->point_id );
      TTlgQry.Execute();
      LogTrace(TRACE5) << iseg->point_id;
      ReadLayers( TTlgQry, filterRoutes, filtersLayers[ iseg->point_id ],
                  pax_lists[ iseg->point_id ],
                  params, true );
    }
  }
      ...
      if ( TReqInfo::Instance()->desk.compatible( TRANSIT_CRAFT_VERSION ) ) {
        ...
        if ( n1 ) {
          ...
          while ( n1 && string( (const char*)n1->name ) == "rfisc" ) {
            ...
            if ( rfisc.color != "clBtnFace" ) { // ошибка на стороне терминала в случае лат. режима
              checkRFISC(  place, rfisc.color, pr_craft_lat, tariffMap, tariffMap.is_rfisc_applied( airline ) );
              place.AddRFISC( point_id, rfisc );
            }
            n1 = n1->next;
...
void TSalonList::WriteFlight( int vpoint_id, bool saveContructivePlaces, bool isLibraRequest )
{
  ...
  make_db_curs("DELETE FROM trip_comp_rem WHERE point_id=:point_id",
               PgOra::getRWSession("TRIP_COMP_REM")).bind(":point_id", vpoint_id).exec();
  make_db_curs("DELETE FROM trip_comp_rfisc WHERE point_id=:point_id",
               PgOra::getRWSession("TRIP_COMP_RFISC")).bind(":point_id", vpoint_id).exec();
  make_db_curs("DELETE FROM trip_comp_baselayers WHERE point_id=:point_id",
               PgOra::getRWSession("TRIP_COMP_BASELAYERS")).bind(":point_id", vpoint_id).exec();
  make_db_curs("DELETE FROM trip_comp_rates WHERE point_id=:point_id",
               PgOra::getRWSession("TRIP_COMP_RATES")).bind(":point_id", vpoint_id).exec();
  make_db_curs("DELETE FROM trip_comp_rfisc WHERE point_id=:point_id",
               PgOra::getRWSession("TRIP_COMP_RFISC")).bind(":point_id", vpoint_id).exec();

  make_db_curs("DELETE FROM trip_comp_elems WHERE point_id=:point_id"
               + ( saveContructivePlaces ? (" AND elem_type NOT IN " + GetSQLEnum(elem_types)) : ""),
...
void TSalonList::WriteCompon( int &vcomp_id, const TComponSets &componSets, bool saveContructivePlaces )
{
  ...
  switch ( (int)componSets.modify ) {
    case mChange:
      ...
      make_db_curs("DELETE FROM comp_rem WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_REM")).stb().bind(":comp_id", vcomp_id).exec();
      make_db_curs("DELETE FROM comp_baselayers WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_BASELAYERS")).stb().bind(":comp_id", vcomp_id).exec();
      make_db_curs("DELETE FROM comp_rates WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_RATES")).stb().bind(":comp_id", vcomp_id).exec();
      make_db_curs("DELETE FROM comp_rfisc WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_RFISC")).stb().bind(":comp_id", vcomp_id).exec();

      if ( saveContructivePlaces ) {
        std::string sqlStr("DELETE FROM comp_elems WHERE comp_id = :comp_id AND elem_type NOT IN ");
        sqlStr += GetSQLEnum(elem_types);
        make_db_curs(sqlStr, PgOra::getRWSession("COMP_ELEMS")).stb().bind(":comp_id", vcomp_id).exec();
      } else {
        make_db_curs("DELETE FROM comp_elems WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_ELEMS")).stb().bind(":comp_id", vcomp_id).exec();
      }
      make_db_curs("DELETE FROM comp_classes WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_CLASSES")).stb().bind(":comp_id", vcomp_id).exec();
      break;
    ...
    case mDelete:
      ...
      make_db_curs("DELETE FROM comp_rfisc WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_RFISC")).stb().bind(":comp_id", vcomp_id).exec();
      make_db_curs("DELETE FROM comp_elems WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_ELEMS")).stb().bind(":comp_id", vcomp_id).exec();
      make_db_curs("DELETE FROM comp_sections WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_SECTIONS")).stb().bind(":comp_id", vcomp_id).exec();
      make_db_curs("DELETE FROM comp_classes WHERE comp_id = :comp_id", PgOra::getRWSession("COMP_CLASSES")).stb().bind(":comp_id", vcomp_id).exec();
      make_db_curs("DELETE FROM comps WHERE comp_id = :comp_id", PgOra::getRWSession("COMPS")).stb().bind(":comp_id", vcomp_id).exec();
...
//      uniqueTariffs.clear();
      ...
      for( std::map<int, TSetOfLayerPriority,classcomp >::iterator ilayers=layers.begin(); ilayers!=layers.end(); ilayers++ ) {
        ...
        for ( TSetOfLayerPriority::iterator ilayer=ilayers->second.begin(); ilayer!=ilayers->second.end(); ilayer++ ) {
          ...
          if ( ilayers->first != getDepartureId() ) { //если это не наш пункт вылета, пакс может встать на Ло при регистрации, если в след. пункте есть слой после месты
            ...
            for ( TSetOfLayerPriority::iterator icurrLayer=currLayers.begin(); //список слоев, которыми планируем разметить место
          ...
          if ( dropLayersFlags.isFlag( clDropBlockCentLayers ) &&
               ...
               ilayer->layerType() == cltBlockCent ) { //удаляем слой блокировки центровки во всех не наших пунктах
  ...
  if ( !dropLayersFlags.isFlag( clDropBlockCentLayers ) &&
       pr_lastRoute &&
       point.point_id != filterRoutes.point_dep ) { //по своему пункту не удаляем блокировку центровки
...
void CheckWaitListToLog( DB::TQuery &QryAirp,
                         bool pr_exists,
                         int pax_id,
                         int point_dep,
...
void TSalonList::check_waitlist_alarm_on_tranzit_routes( const std::set<int> &paxs_external_logged  )
{
  ...
  DB::TQuery QryAirp( PgOra::getROSession("POINTS"),STDLOG );
  QryAirp.SQLText =
...
bool TSalonList::check_waitlist_alarm_on_tranzit_routes( const TAutoSeats &autoSeats )
{
  ...
  for ( TAutoSeats::const_iterator ipass=autoSeats.begin();
    ...
    for ( TLayersPax::iterator ilayer=ipax->second.layers.begin();
          ilayer!=ipax->second.layers.end(); ilayer++ ) {
      if ( ilayer->second.waitListReason.status != layerValid ) { //инвалидные слои не могут влиять???
        ...
        for ( TSetOfLayerPriority::iterator ilayer=ilayers->second.begin();
          ...
          if ( (ilayer->getPaxId() != ASTRA::NoExists && /* слой принадлежит другому пассажиру */
                ...
                (ilayer->getPaxId() == ASTRA::NoExists &&
                 menuLayers[ ilayer->layerType() ].notfree) /*слой не принадлежит пассажиру и недоступен */
             ) {
  ...
  for ( TSetOfLayerPriority::const_iterator ip1_layer=p1_layers[ point_id ].begin();
    ...
    for ( TSetOfLayerPriority::const_iterator ip2_layer=p2_layers[ point_id ].begin();
      ...
      if ( !editabeLayers.isFlag( ip2_layer->layerType() ) ) {
        ...
        if ( ip1_layer->layerType() == ip2_layer->layerType() ) { //возможно нужно сравнение и других атрибутов слоя??? - в будущем для разметки по маршруту
        pr_find_layer = true;
        break;
      }
    }
...
void getXYName( int point_id, std::string seat_no, std::string &xname, std::string &yname )
{
    ...
    DB::TQuery Qry(PgOra::getROSession("TRIP_COMP_RANGES"),STDLOG);
    Qry.SQLText =
...
bool filterComponsForView( const string &airline, const string &airp )
{
  if ((airline.empty() && airp.empty()) ||
      (!airline.empty() && !airp.empty())) return false;  //компоновка может принадлежать либо порту, либо компании
...
void TComponSets::CheckAirlAirp( xmlNodePtr reqNode, string &airline, string &airp )
{
  ...
  if (!airline.empty() && !airp.empty())
    throw AstraLocale::UserException( "MSG.NOT_SET_ONE_TIME_AIRLINE_AND_AIRP" ); // потому что компоновка принадлежит либо авиакомпании, либо порту
...
void DeleteSalons( int point_id )
{
    ...
    make_db_curs("DELETE FROM trip_comp_rem WHERE point_id = :point_id", PgOra::getRWSession("TRIP_COMP_REM")).stb().bind(":point_id", point_id).exec();
    make_db_curs("DELETE FROM trip_comp_baselayers WHERE point_id = :point_id", PgOra::getRWSession("TRIP_COMP_BASELAYERS")).stb().bind(":point_id", point_id).exec();
    make_db_curs("DELETE FROM trip_comp_rates WHERE point_id = :point_id", PgOra::getRWSession("TRIP_COMP_RATES")).stb().bind(":point_id", point_id).exec();
    make_db_curs("DELETE FROM trip_comp_rfisc WHERE point_id = :point_id", PgOra::getRWSession("TRIP_COMP_RFISC")).stb().bind(":point_id", point_id).exec();
    make_db_curs("DELETE FROM trip_comp_elems WHERE point_id = :point_id", PgOra::getRWSession("TRIP_COMP_ELEMS")).stb().bind(":point_id", point_id).exec();
    ComponCreator::setFlightClasses( point_id );
}

bool isEmptySalons( int point_id )
{
  return !getPrSalon(point_id); // alarms.cc
}
...
void TSalonPax::int_get_seats( TWaitListReason &waitListReason,
  ...
  if ( ilayer != layers.end() ) {
    ...
    if ( ilayer->second.waitListReason.status == layerValid ) { //нашли слой
      waitListReason = ilayer->first; //возвращаем слой
...
void TSalonPax::get_seats( TWaitListReason &waitListReason,
                           TPassSeats &ranges) const {
  TCompLayerType pax_layer_type;
  get_seats(waitListReason,pax_layer_type,ranges);
}
...
std::string TSalonPax::prior_seat_no( const std::string &format, bool pr_lat_seat) const
{
  ...
  const TGrpStatusTypesRow &grp_status_row = (const TGrpStatusTypesRow&)grp_status_types.get_row( "code", grp_status );
  grp_layer_type = DecodeCompLayerType( grp_status_row.layer_type.c_str() );
  for ( const auto &irange : total_ranges ) { // а нужен ли здесь пробег по слоям или самы первый-приоритетный?
    LogTrace(TRACE5)  << irange.first.toString() << " " <<EncodeCompLayerType(grp_layer_type);
    if ( irange.first.layerType() == grp_layer_type ) {
      int vseats = seats;
      return GetSeatRangeView(irange.second, format, pr_lat_seat, vseats, false);
    }
  }
  return "";
}
                                          ...
                                          TCompLayerType& pax_layer_type) const
{
  ...
  pax_layer_type = iranges->first.layerType();
...
void TAutoSeats::WritePaxSeats( int point_dep, int pax_id )
{
  ...
  DB::TQuery QryIns(PgOra::getRWSession("PAX_SEATS"),STDLOG);
  QryIns.SQLText =
    "INSERT INTO pax_seats(point_id,pax_id,xname,yname,pr_wl,seat_descr) "
    "VALUES(:point_id,:pax_id,:xname,:yname,0,:seat_descr)";
  QryIns.CreateVariable( "point_id", otInteger, point_dep );
  QryIns.CreateVariable( "pax_id", otInteger, ipax->pax_id );
  QryIns.DeclareVariable( "xname", otString );
  QryIns.DeclareVariable( "yname", otString );
  QryIns.DeclareVariable( "seat_descr", otString );
  for ( std::vector<TSeat>::iterator irange=ipax->ranges.begin();
        irange!=ipax->ranges.end(); irange++ ) {
    QryIns.SetVariable( "xname", irange->line );
    QryIns.SetVariable( "yname", irange->row );
    QryIns.SetVariable( "seat_descr", ipax->descrs[ *irange ] );
    QryIns.Execute();
  }
}
...
const TAdjustmentRows &TAdjustmentRows::get( const TSalonList &salonList ) {
  ...
  int idx = 0;
  for ( auto isalon=salonList._seats.begin(); isalon!=salonList._seats.end(); isalon++ ) { // интересны последние ряды, которые могут не отображаться
