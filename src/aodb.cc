  ...
  public:
             ...
             TBagNamesItem& item) const
    {
      ...
      for(TBagNamesList::const_iterator i=begin(); i!=end(); ++i)
      {
        ...
        if (curr_cost==ASTRA::NoExists) continue; //нам не подходит
  ...
  try {
    ...
    if ( pr_insert ) {
      ...
      if ( franchise_prop.get_franchisee(info, Franchise::TPropType::aodb) &&
        ...
        ProgTrace( TRACE5, "airline=%s, %d, flt_no=%d, suffix=%s, pr_insert=%d", fl.airline.code.c_str(), fl.airline.code == "ЮТ", fl.flt_no, fl.suffix.code.c_str(), pr_insert );
      }
    }
    if ( pr_insert ) {
      ProgTrace( TRACE5, "ParseFlight: new flight - return" );
                     ...
                     TAODBFormat format, TFileDatas &fds )
{
  ...
  StationsQry.CreateVariable("work_mode", otString, "Р");
  StationsQry.Execute();
