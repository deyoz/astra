...
void get_stat(const PointId_t& point_id)
{
  if(DEMO_MODE())
  {
      // В ДЕМО ОБХОДИМ СБОР СТАТИСТИКИ СТОРОНОЙ
      TST();
      return;
  }
  ...
  DB::TQuery Qry(PgOra::getROSession({"PAX","PAX_GRP","TCKIN_PAX_GRP","BAG2","EVENTS_BILINGUAL","STATIONS"}), STDLOG);
  Qry.SQLText =
      "SELECT "
      "  pax.pax_id, "
      "  airp_arv, "
      ...
      "  ) AS term_bag, "
      "  (SELECT 1 FROM events_bilingual "
      "   JOIN stations ON events_bilingual.station = stations.desk "
      "   WHERE "
      "      stations.work_mode = 'Р' "
      "      AND lang = 'RU' "
      "      AND type IN (:evt_pax, :evt_pay) "
      "      AND id1 = pax_grp.point_dep "
      "      AND id2 = pax.reg_no "
      "   FETCH FIRST 1 ROWS ONLY "
      "  ) AS term_ckin_service "
      "FROM pax_grp "
      "  LEFT OUTER JOIN tckin_pax_grp ON (tckin_pax_grp.grp_id = pax_grp.grp_id "
      "                                    AND tckin_pax_grp.transit_num <> 0) "
      "  JOIN pax ON pax_grp.grp_id = pax.grp_id "
      "WHERE  "
      "  point_dep = :point_id "
      "  AND pax_grp.status NOT IN ('E') "
      "  AND NOT (pax_grp.status IN ('T') AND tckin_pax_grp.grp_id IS NOT NULL) "
...
void get_self_ckin_stat(const PointId_t& point_id)
{
  ...
  Qry.SQLText =
      ...
      "  ) AS term_ckin_service "
      "FROM pax_grp "
      "  JOIN web_clients ON pax_grp.user_id = web_clients.user_id "
      "  JOIN desks ON web_clients.desk = desks.code "
      "  JOIN desk_grp ON desks.grp_id = desk_grp.grp_id "
      "  LEFT OUTER JOIN tckin_pax_grp "
      "    ON ( "
      "      pax_grp.grp_id = tckin_pax_grp.grp_id "
      "      AND tckin_pax_grp.seg_no = 1 "
      "      AND tckin_pax_grp.transit_num = 0 "
      "    ) "
      "  JOIN pax ON pax_grp.grp_id = pax.grp_id "
      "WHERE "
      ...
      "  AND pax_grp.status NOT IN ('E') "
      "  AND web_clients.client_type IN ('KIOSK', 'WEB', 'MOBIL') ";
  Qry.CreateVariable("point_id", otInteger, point_id.get());
  Qry.CreateVariable("evt_pax", otString, EncodeEventType(evtPax));
  Qry.CreateVariable("evt_pay", otString, EncodeEventType(evtPay));
  Qry.Execute();
