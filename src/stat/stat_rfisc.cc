...
void get_rfisc_stat(int point_id)
{
    ...
    DB::TCachedQuery bagQry(
          PgOra::getROSession({"PAX_GRP","POINTS","BAG2","PAX","TRANSFER","TRFER_TRIPS","USERS2"}),
          "SELECT "
          "    points.point_id, "
          ...
          "JOIN points arv_point ON pax_grp.point_arv = arv_point.point_id AND arv_point.pr_del >= 0 "
          "JOIN (bag2 "
          "    LEFT OUTER JOIN pax ON bag2.grp_id = pax.grp_id "
          "    LEFT OUTER JOIN users2 ON bag2.user_id = users2.user_id "
        ...
        )
{
    ...
    ArxRunRFISCStat(params, RFISCStat, prn_airline);
}
