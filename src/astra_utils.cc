...
void LoadRemGrp(TRemEventType rem_set_type, int point_id, vector<string> &result)
{
    string event_type;
    switch(rem_set_type) {
        case retALARM_SS:
    ...
    for(; not Qry.Eof; Qry.Next())
        if(Qry.FieldAsInteger("event_value"))
            result.push_back(Qry.FieldAsString("rem_code"));
    if(not processed) {
        Qry.Clear();
        Qry.SQLText =
