...
namespace json_spirit
{
    ...
    void unicode_str_to_char( Iter_type& begin, String_type& s )
    {
        const typename String_type::value_type c1( hex_to_num(*( ++begin )) );
        const typename String_type::value_type c2( hex_to_num(*( ++begin )) );
        const typename String_type::value_type c3( hex_to_num(*( ++begin )) );
        const typename String_type::value_type c4( hex_to_num(*( ++begin )) );
        // Make UTF8 character (only [one|two] bytes)
        // Native library code "return String_type::value_type(( c1 << 12 ) + ( c2 <<  8 ) + ( c3 <<  4 ) + c4);" fail
        // http://en.wikipedia.org/wiki/UTF-8
        if (!c1 && !c2) {
            s += (c3 << 0x04) | c4;
        } else if (!c1 && c2) {
            s += (0xC0 | ((c2 << 0x02) & 0x1C)) | ((c3 & 0x0C) >> 0x02);
            s += (0x80 | (c3 & 0x03) << 0x04) | c4;
        }
    }
                                        ...
                                        typename String_type::const_iterator end )
    {
        ...
        switch( c2 )
        {
            ...
            case 'u':
            {
                ...
                {
                    unicode_str_to_char< String_type >( begin, s );
                }
                break;
            }
        }
