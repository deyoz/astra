...
static const char* trtab_1_1[256] = {
             "",     "\x01",     "\x02",     "\x03",
         "\x04",     "\x05",     "\x06",     "\x07",
         "\x08",     "\x09",     "\x0a",     "\x0b",
         "\x0c",     "\x0d",     "\x0e",     "\x0f",
...
std::string translit(const std::string& str, bool one_to_one)
{
    std::string result;
    const char** tr_tab = one_to_one ? trtab_1_1 : trtab;
    // Оставим запас в несколько символов, т.к. длина строки может увеличиться при транслитерации
    result.reserve(str.size() + 10);

    for (std::string::const_iterator c = str.begin(); c != str.end(); ++c)
        result += tr_tab[(unsigned char)*c];
