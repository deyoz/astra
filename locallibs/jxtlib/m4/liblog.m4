...
[
   ...
   if test "x$want_liblog" = "xyes"; then
      ...
      LDFLAGS="$LDFLAGS $BOOST_LDFLAGS $BOOST_SYSTEM_LIB $BOOST_FILESYSTEM_LIB $BOOST_THREAD_LIB $LIBLOG_LDFLAGS"
      export LDFLAGS

      AC_CACHE_CHECK(whether logging library is available, ax_cv_liblog,
           [AX_CXX_LT_LINK_IFELSE([AC_LANG_PROGRAM([[@%:@include <log/log.hpp>]], [[return 0;]])],
