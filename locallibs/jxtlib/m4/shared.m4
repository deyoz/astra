...
[
  ...
  shared_depend_libs="$($SHARED/dependlibs-config --ldflags)"
  shared_cflags="-I$SHARED/src"
  shared_ldflags="-L$SHARED/src"
  shared_libs="-lshared"

  AC_SUBST(SHARED_DEPEND_LIBS, $shared_depend_libs)
  AC_SUBST(SHARED_CFLAGS, $shared_cflags)
  AC_SUBST(SHARED_LDFLAGS, $shared_ldflags)
  AC_SUBST(SHARED_LIBS, $shared_libs)
